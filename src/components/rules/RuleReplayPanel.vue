	...
	<section class="ruleReplay">
		...
		<p class="lead">
			{{
				t(
					'openregister',
					'This previews the run. It writes nothing until you commit the job it creates.',
				)
			}}
		</p>
		...
		<div class="replayField">
			<NcTextField
				...
				:placeholder="
					t('openregister', 'Leave empty to search every register')
				" />
			<label for="ruleReplayQuery">{{
				t('openregister', 'Which objects, as a JSON query')
			}}</label>
			<textarea
				id="ruleReplayQuery"
				v-model="query"
			...
			<NcTextField
				...
				:placeholder="
					t(
						'openregister',
						'The termijn changed, so every open case needs the new date',
					)
				" />
		</div>
		...
		<div v-if="result" class="replayResult">
			<p>
				{{
					t('openregister', 'This would touch {count} objects.', {
						count: result.count,
					})
				}}
				<span v-if="result.maxObjects">{{
					t('openregister', 'The ceiling on this rule is {max}.', {
						max: result.maxObjects,
					})
				}}</span>
				<span v-else class="muted">{{
					t('openregister', 'This rule declares no ceiling.')
				}}</span>
			</p>
			<p>
				{{
					t(
						'openregister',
						'Job {id} is waiting for you to commit it, on the bulk jobs page.',
						{ id: result.job.id },
					)
				}}
			</p>
		...
		async run() {
			...
			try {
				...
				this.refusal = failure?.response?.data?.error ?? {
					message: messageFor(
						failure,
						t('openregister', 'The replay could not be previewed.'),
					),
				}
			} finally {
				this.running = false
