	...
	<div class="approval-step-list">
		...
		<div v-for="step in steps" :key="step.id" class="step-row">
			...
			<span v-if="step.decidedBy" class="decided-by"
				>{{ t('openregister', 'by') }} {{ step.decidedBy }}</span
			>
			<div
				v-if="step.status === 'pending' && canDecide(step)"
				class="step-actions">
				<input
					v-model="comments[step.id]"
					type="text"
...
export default {
	...
	methods: {
		...
		async fetchSteps() {
			try {
				...
				const response = await axios.get(url, {
					params: { objectUuid: this.objectUuid },
				})
		...
		async approve(step) {
			try {
				const url = generateUrl(
					`/apps/openregister/api/approval-steps/${step.id}/approve`,
				)
		...
		async reject(step) {
			try {
				const url = generateUrl(
					`/apps/openregister/api/approval-steps/${step.id}/reject`,
				)
				await axios.post(url, { comment: this.comments[step.id] || '' })
				this.fetchSteps()
			} catch (error) {
