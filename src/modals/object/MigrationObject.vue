	...
	<NcDialog :name="`Migrate ${selectedObjects.length} object${selectedObjects.length !== 1 ? 's' : ''}`"
			...
			<div class="source-info">
				...
				<div class="info-card">
					<div class="card-item">
						...
						<span class="card-value">{{ registerStore.item?.title || registerStore.item?.id }}</span>
					</div>
					<div class="card-item">
						<div class="card-label-with-icon">
						...
						<span class="card-value">{{ schemaStore.item?.title || schemaStore.item?.id }}</span>
					</div>
				</div>
			</div>
		...
		<div v-if="step === 3" class="migration-step">
			...
			<NcNoteCard type="info">
				...
				<strong>{{ schemaStore.item?.title }}</strong> to target schema <strong>{{ targetSchema?.title }}</strong>
			</NcNoteCard>
			...
			<div class="mapping-container">
				<div class="mapping-header">
					<div class="source-header">
						...
						<span class="schema-name">{{ schemaStore.item?.title }}</span>
					</div>
					...
					<div class="migration-info">
						<div class="migration-detail">
							...
							<div class="migration-meta">
								<span>{{ registerStore.item?.title }} / {{ schemaStore.item?.title }}</span>
...
export default {
	...
	methods: {
		...
		async loadSchemaProperties() {
			if (!schemaStore.item || !this.targetSchema) {
				return
			}

			try {
				// Load source schema properties
				this.sourceProperties = this.extractSchemaProperties(schemaStore.item)
		...
		async performMigration() {
			...
			try {
				const response = await fetch('/index.php/apps/openregister/api/migrate', {
					...
					body: JSON.stringify({
						sourceRegister: registerStore.item.id,
						sourceSchema: schemaStore.item.id,
