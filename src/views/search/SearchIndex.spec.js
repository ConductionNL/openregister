...
import { objectStore as mockObjectStore, viewsStore as mockViewsStore } from '../../store/store.js'
...
jest.mock('@nextcloud/l10n', () => ({ __esModule: true, translate: (_app, text) => text }))
...
jest.mock('vue-material-design-icons/ContentCopy.vue', () => ({ __esModule: true, default: { name: 'icon', render: () => null } }), { virtual: true })
...
describe('SearchIndex — presentation dispatch (REQ-VIEW-PRES-05)', () => {
	beforeEach(() => {
		Object.assign(mockObjectStore, {
			searchCollection: [],
			searchParams: { register: null, schema: null, sortKey: null, sortOrder: null },
	...
	it('presentationType reflects a kanban view', () => {
		const ctx = { activeView: { id: 'v1', presentation: { viewType: 'kanban', kanban: { groupByField: 'status' } } } }
		expect(computedCall('presentationType', ctx)).toBe('kanban')
	})

	it('presentationType reflects a calendar view', () => {
		const ctx = { activeView: { id: 'v1', presentation: { viewType: 'calendar', calendar: { dateField: 'dueDate' } } } }
		expect(computedCall('presentationType', ctx)).toBe('calendar')
	})

	it('activeViewId falls back to uuid when id is absent', () => {
		expect(computedCall('activeViewId', { activeView: { uuid: 'u-1' } })).toBe('u-1')
	...
	it('normalizedKanbanColumns normalizes each column\'s cards (top-level id from @self.id)', () => {
		const ctx = {
			kanbanBoard: {
				columns: [
					{ value: 'todo', cards: [{ '@self': { id: 'obj-1' }, title: 'Card 1' }], total: 1, limit: 20, offset: 0 },
					{ value: 'done', cards: [{ id: 'obj-2', title: 'Card 2' }], total: 1, limit: 20, offset: 0 },
				],
			},
		}
	...
	it('normalizedKanbanColumns is empty when there is no board yet', () => {
		expect(computedCall('normalizedKanbanColumns', { kanbanBoard: null })).toEqual([])
	})

	it('normalizedCalendarObjects normalizes objects the same way as the table', () => {
		const ctx = { calendarObjects: [{ '@self': { id: 'obj-3' }, dueDate: '2026-07-24' }] }
	...
	describe('handleKanbanMove (REQ-VIEW-KANBAN-03)', () => {
		it('on a legal move, saves the full object (PUT-semantic) and resolves the optimistic move', async () => {
			const card = { id: 'obj-1', '@self': { id: 'obj-1' }, status: 'todo', title: 'Card 1' }
			mockObjectStore.saveObject.mockResolvedValue({ ...card, status: 'doing' })
			...
			await methodCall('handleKanbanMove', ctx, { object: card, groupByField: 'status', fromValue: 'todo', toValue: 'doing' })

			// Every existing field is carried forward — only groupByField changes.
			expect(mockObjectStore.saveObject).toHaveBeenCalledWith('reg-1-schema-1', { ...card, status: 'doing' })
		...
		it('on an illegal lifecycle transition, rejects with the server\'s reason so the card snaps back', async () => {
			...
			mockObjectStore.errors = { 'reg-1-schema-1': { message: 'Illegal transition: done -> todo' } }
			...
			await methodCall('handleKanbanMove', ctx, { object: card, groupByField: 'status', fromValue: 'done', toValue: 'todo' })

			expect(rejectMove).toHaveBeenCalledWith('obj-2', 'Illegal transition: done -> todo')
		...
		it('falls back to a generic reason when the server error carries no message', async () => {
			...
			const rejectMove = jest.fn()
			const ctx = { computedObjectType: 'reg-1-schema-1', $refs: { kanban: { resolveMove: jest.fn(), rejectMove } } }

			await methodCall('handleKanbanMove', ctx, { object: { id: 'obj-3' }, groupByField: 'status', toValue: 'todo' })
	...
	describe('handleKanbanLoadMore (REQ-VIEW-KANBAN-02)', () => {
		it('refetches the board at a larger limit and swaps in only the requesting column', async () => {
			mockViewsStore.fetchKanbanBoard.mockResolvedValue({
				columns: [
					{ value: 'todo', cards: [{ id: '1' }, { id: '2' }], total: 5, limit: 40, offset: 0 },
					{ value: 'done', cards: [{ id: '9' }], total: 1, limit: 40, offset: 0 },
				],
			})
			const ctx = {
				activeViewId: 'v1',
				...
				kanbanBoard: {
					columns: [
						{ value: 'todo', cards: [{ id: '1' }], total: 5, limit: 20, offset: 0 },
						{ value: 'done', cards: [{ id: '9' }], total: 1, limit: 20, offset: 0 },
					],
				},
			}

			await methodCall('handleKanbanLoadMore', ctx, { value: 'todo', offset: 1 })

			expect(mockViewsStore.fetchKanbanBoard).toHaveBeenCalledWith('v1', { _limit: 40 })
			// Only the 'todo' column was replaced with the refetched result.
			expect(ctx.kanbanBoard.columns.find((c) => c.value === 'todo').cards).toHaveLength(2)
			expect(ctx.kanbanBoard.columns.find((c) => c.value === 'done').cards).toHaveLength(1)
		...
		it('does nothing when there is no active view', async () => {
			...
			await methodCall('handleKanbanLoadMore', ctx, { value: 'todo', offset: 0 })
	...
	describe('handleCalendarRangeChange (REQ-VIEW-CAL-04)', () => {
		it('fetches objects for the visible range and stores them', async () => {
			mockViewsStore.fetchCalendarObjects.mockResolvedValue({ objects: [{ id: 'o1', dueDate: '2026-07-10' }] })
			const ctx = { activeViewId: 'v2', calendarLoading: false, calendarObjects: [] }

			await methodCall('handleCalendarRangeChange', ctx, { rangeStart: '2026-07-01', rangeEnd: '2026-07-31' })

			expect(mockViewsStore.fetchCalendarObjects).toHaveBeenCalledWith('v2', '2026-07-01', '2026-07-31')
			expect(ctx.calendarObjects).toEqual([{ id: 'o1', dueDate: '2026-07-10' }])
