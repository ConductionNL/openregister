							...
							<div class="solr-core-card">
								...
								<div class="core-info">
									...
									<div class="core-detail">
										...
										<span class="detail-value" :class="getCoreStatusClass(solrStats.cores && solrStats.cores.core_status)">{{ solrStats.cores && solrStats.cores.core_status || 'Unknown' }}</span>
									</div>
									<div class="core-detail">
...
export default {
	...
	computed: {
		 ...
		 */
		performanceClass() {
			console.log('[PERFORMANCE CLASS] Called with solrStats:', this.solrStats)
			if (!this.solrStats || !this.solrStats.performance) {
				console.log('[PERFORMANCE CLASS] Returning performance-low due to missing data')
				return 'performance-low'
			}
			const opsPerSec = this.solrStats.performance.operations_per_sec || 0
			console.log('[PERFORMANCE CLASS] opsPerSec:', opsPerSec)
			...
			try {
				...
				if (response.data && response.data.available) {
					...
					this.solrStats = {
						...
						overview: {
							connection_status: response.data.available ? 'Connected' : 'Disconnected',
							response_time_ms: 0, // Not provided by current API
							total_documents: response.data.document_count || 0,
							index_size: 'Unknown', // Not provided by current API
						},
						performance: {
							operations_per_sec: 0, // Calculate from service_stats if needed
							total_searches: response.data.service_stats?.searches || 0,
							avg_search_time_ms: response.data.service_stats?.search_time || 0,
							total_indexes: response.data.service_stats?.indexes || 0,
							avg_index_time_ms: response.data.service_stats?.index_time || 0,
							error_rate: 0, // Calculate from service_stats if needed
							total_deletes: response.data.service_stats?.deletes || 0,
						},
						cores: {
							active_core: response.data.collection || 'Unknown',
							core_status: response.data.available ? 'Active' : 'Inactive',
							tenant_id: response.data.tenant_id || 'Unknown',
							endpoint_url: 'Unknown', // Not provided by current API
						},
						health: {
							status: response.data.health || 'unknown',
							uptime: 'Unknown', // Not provided by current API
							last_optimization: response.data.last_modified || null,
							memory_usage: {
								used: 'N/A',
								max: 'N/A',
								percentage: 0,
					...
					// Reset to default structure when there's an error
					this.solrStats = {
						available: false,
						overview: {
