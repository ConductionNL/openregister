...
const PACK = {
	...
	denialGrounds: [
		{ key: 'manifestly-unfounded', label: 'Manifestly unfounded', citation: 'GDPR Art 12(5)' },
		{ key: 'third-party-rights', label: 'Third-party rights', citation: 'GDPR Art 15(4)' },
	],
...
describe('AVG store — DSAR case management', () => {
	...
	describe('label resolvers (pack-driven, no inlined jurisdiction strings)', () => {
		...
		it('maps denial grounds to options with label + citation', () => {
			...
			expect(opts[0]).toEqual({ value: 'manifestly-unfounded', label: 'Manifestly unfounded', citation: 'GDPR Art 12(5)' })
		})
		...
		it('resolves a template reference (leaf) from the pack, never inline body', () => {
			expect(resolveTemplateRef(PACK, 'denial')).toBe('template:denial-default')
			expect(resolveTemplateRef(PACK, 'acknowledgement')).toBe('')
		})

		it('honours a pack statusLabels override, else humanises the key', () => {
			expect(resolveStatusLabel({ statusLabels: { received: 'Ontvangen' } }, 'received')).toBe('Ontvangen')
			expect(resolveStatusLabel(null, 'evidence-collection')).toBe('Evidence collection')
		})
	})

	describe('lifecycle transition mirror', () => {
		it('offers finaliseDenial only from denial-drafted', () => {
			const fromDrafted = CASE_LIFECYCLE_TRANSITIONS.filter((tr) => tr.from.includes('denial-drafted'))
			expect(fromDrafted.map((t) => t.action)).toContain('finaliseDenial')
			const fromReceived = CASE_LIFECYCLE_TRANSITIONS.filter((tr) => tr.from.includes('received'))
	...
	describe('fetchCases', () => {
		it('lists cases via the objects API and stores the results', async () => {
			axios.get.mockResolvedValueOnce({ data: { results: [{ uuid: 'c1', status: 'received' }] } })
			const store = useAvgStore()
			const cases = await store.fetchCases({ status: 'received' })
			expect(axios.get).toHaveBeenCalledWith(
				expect.stringContaining('/api/objects/data-subject-requests/dataSubjectRequest'),
	...
	describe('transitionCase', () => {
		it('posts the action to the transition endpoint and stores the returned case', async () => {
			axios.post.mockResolvedValueOnce({ data: { uuid: 'c1', status: 'verifying' } })
			const store = useAvgStore()
		...
		it('surfaces a server guard refusal as an error and rethrows', async () => {
			axios.post.mockRejectedValueOnce({ response: { data: { error: 'regulatorReference required' } } })
			const store = useAvgStore()
			await expect(store.transitionCase('c1', 'finaliseDenial')).rejects.toBeDefined()
	...
	describe('draftDenial', () => {
		it('records the ground then posts the draftDenial transition', async () => {
			axios.put.mockResolvedValueOnce({ data: { uuid: 'c1', denialGround: 'third-party-rights' } })
			axios.post.mockResolvedValueOnce({ data: { uuid: 'c1', status: 'denial-drafted' } })
			const store = useAvgStore()
			await store.draftDenial('c1', 'third-party-rights')
			expect(axios.put).toHaveBeenCalledWith(
				expect.stringContaining('/api/objects/data-subject-requests/dataSubjectRequest/c1'),
	...
	describe('generateBundle', () => {
		it('posts to the bundle endpoint and returns the one-time token metadata', async () => {
			axios.post.mockResolvedValueOnce({ data: { downloadToken: 'YOUR_TOKEN_HERE', signed: true } })
			const store = useAvgStore()
			const meta = await store.generateBundle('c1')
			expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/api/gdpr/cases/c1/bundle'))
	...
	describe('verifyIdentity (fail-closed seam)', () => {
		it('returns the seam three-state result faithfully', async () => {
			axios.post.mockResolvedValueOnce({ data: { status: 'needs-more', provider: 'or.default.identity-verify.null' } })
			const store = useAvgStore()
			const result = await store.verifyIdentity('c1')
			expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/api/gdpr/cases/c1/verify-identity'))
	...
	describe('escalateRegulator (fail-closed seam)', () => {
		it('returns the seam performed/refused result faithfully', async () => {
			axios.post.mockResolvedValueOnce({ data: { status: 'refused', provider: 'or.default.regulator-escalate.null' } })
			const store = useAvgStore()
	...
	describe('fetchActivePolicyPack', () => {
		it('selects the pack matching the requested jurisdiction', async () => {
			...
			const store = useAvgStore()
			const pack = await store.fetchActivePolicyPack({ jurisdiction: 'nl-example' })
		...
		it('falls back to the default pack when the jurisdiction has none', async () => {
			...
			const store = useAvgStore()
			const pack = await store.fetchActivePolicyPack({ jurisdiction: 'unknown' })
