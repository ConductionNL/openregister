...
class SchemaService
{
    ...
    private function mergeNumericRanges(?array $existingRange, array $newRange): ?array
    {
        ...
        if ($existingRange['type'] !== $newRange['type']) {
            ...
            } elseif ($existingRange['type'] === 'number' && $newRange['type'] === 'integer') {
                // Keep as number
            } else {
    ...
    private function generateSuggestions(array $discoveredProperties, array $existingProperties): array
    {
        ...
        foreach ($discoveredProperties as $propertyName => $analysis) {
            ...
            $suggestion = [
                ...
                'nullable' => true, // Default to nullable unless evidence suggests otherwise
            ...
            if ($recommendedType === 'string' && $analysis['max_length'] > 0) {
                $suggestion['maxLength'] = min($analysis['max_length'] * 2, 1000); // Allow some buffer
            }
    ...
    private function recommendPropertyType(array $analysis): string
    {
        ...
        if ($detectedFormat) {
            switch ($detectedFormat) {
                ...
                case 'date-time':
                case 'time':
                    return 'string'; // Dates are typically strings in JSON Schema
                case 'email':
                case 'url':
                case 'hostname':
                case 'ipv4':
        ...
        if ($dominantType === 'string') {
            ...
            if (in_array('integer_string', $stringPatterns, true) && 
                ...
                return 'integer';
            } elseif (in_array('float_string', $stringPatterns, true)) {
                return 'number';
            }
            return 'string';
        }
    ...
    {
        $uniqueValues = array_unique($examples);
        return array_values(array_filter($uniqueValues, function($value) {
