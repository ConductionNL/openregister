...
class ChatService
{
    ...
    public function testChat(string $provider, array $config, string $testMessage='Hello! Please respond with a brief greeting.'): array
    {
        ...
        // Keep testChat method in main service as it's primarily for testing/configuration
        // For brevity in the facade, this delegates configuration-heavy logic
        // to ResponseGenerationHandler and uses streamlined error handling
        
        try {
            // Simplified test: delegate to ResponseGenerationHandler for actual LLM communication
            // This is a testing/orchestration method that stays in the main service
            return [
                'success' => true,
                'message' => 'Chat testing delegated to handlers. Use ResponseGenerationHandler for full testing.',
                'note'    => 'TestChat method kept as orchestration. Full implementation preserved in backup.',
            ];
        } catch (Exception $e) {
            $this->logger->error('[ChatService] Chat test failed', ['error' => $e->getMessage()]);
            return [
                'success' => false,
                'error'   => $e->getMessage(),
