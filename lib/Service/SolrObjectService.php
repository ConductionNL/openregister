...
class SolrObjectService
{
    ...
    public function convertObjectToText(ObjectEntity $object): string
    {
        ...
        try {
            ...
            $this->logger->debug('Could not load schema for object', [
        ...
        try {
            ...
            $this->logger->debug('Could not load register for object', [
                'object_id' => $object->getId(),
        ...
        $this->logger->debug('Converted object to text for embedding', [
            'object_id' => $object->getId(),
    ...
    public function convertObjectsToText(array $objects): array
    {
        ...
        foreach ($objects as $object) {
            if (!$object instanceof ObjectEntity) {
                $this->logger->warning('Skipping non-ObjectEntity in batch conversion', [
            ...
            try {
                ...
                $this->logger->error('Failed to convert object to text', [
                    'object_id' => $object->getId(),
                    'error' => $e->getMessage()
                ]);
            }
        ...
        $this->logger->info('Batch converted objects to text', [
    ...
    public function indexObject(ObjectEntity $object, bool $commit = false): bool
    {
        ...
        $this->logger->info('Indexing object to objectCollection', [
            'object_id' => $object->getId(),
    ...
    public function bulkIndexObjects(array $objects, bool $commit = true): array
    {
        ...
        $this->logger->info('Bulk indexing objects to objectCollection', [
    ...
    public function searchObjects(array $query = [], bool $rbac = true, bool $multi = true, bool $published = false, bool $deleted = false): array
    {
        ...
        $this->logger->debug('Searching objects in objectCollection', [
            'collection' => $collection,
            'query' => $query,
            'rbac' => $rbac,
            'multi' => $multi,
    ...
    public function deleteObject(string|int $objectId, bool $commit = false): bool
    {
        ...
        $this->logger->info('Deleting object from objectCollection', [
            'object_id' => $objectId,
            'collection' => $collection
        ]);
    ...
    public function warmupObjects(array $schemaIds = [], int $maxObjects = 0, int $batchSize = 1000): array
    {
        ...
        $this->logger->info('Warming up objectCollection', [
            'collection' => $collection,
            'schemaIds' => $schemaIds,
    ...
    public function reindexObjects(int $maxObjects = 0, int $batchSize = 1000, array $schemaIds = []): array
    {
        ...
        $this->logger->info('Reindexing objects to objectCollection', [
            'collection' => $collection,
            'maxObjects' => $maxObjects,
            'batchSize' => $batchSize
        ]);
    ...
    public function clearObjectIndex(): array
    {
        ...
        $this->logger->warning('Clearing all objects from objectCollection', [
            'collection' => $collection
        ]);
    ...
    public function vectorizeObject(ObjectEntity $object, ?string $provider = null): array
    {
        ...
        $vectorId = $vectorService->storeVector(
            'object',                      // entity_type
            (string)$object->getId(),      // entity_id
            $embedding['embedding'],       // embedding array
            $embedding['model'],           // model name
            $embedding['dimensions'],      // dimensions
            0,                             // chunk_index (objects are not chunked)
            1,                             // total_chunks (always 1 for objects)
            $text,                         // chunk_text
            [
                'uuid' => $object->getUuid(),
                'schema_id' => $object->getSchema(),
        ...
        $this->logger->info('Object vectorized successfully', [
            'object_id' => $object->getId(),
            'uuid' => $object->getUuid(),
            'vector_id' => $vectorId,
    ...
    public function vectorizeObjects(array $objects, ?string $provider = null): array
    {
        ...
        $this->logger->info('Starting batch object vectorization', [
            'total_objects' => count($objects),
        ...
        foreach ($textData as $index => $item) {
            ...
            try {
                ...
                $this->logger->error('Failed to vectorize object', [
                    'object_id' => $item['object_id'],
                    'error' => $e->getMessage()
                ]);
            }
        ...
        $this->logger->info('Batch object vectorization complete', [
            'total' => count($objects),
