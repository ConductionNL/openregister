 ...
 */
class GuzzleSolrService implements \OCA\OpenRegister\Service\Index\SearchBackendInterface
{
    /**
     ...
     *
     * Used for making HTTP requests to SOLR server.
     * Currently uses GuzzleClient, will switch to IClient in future.
     *
     ...
     *
     * Configuration array containing SOLR connection details and settings.
     *
     ...
     *
     * Stores field type information from SOLR to reduce API calls.
     *
     * @var array|null Array of field types or null if not cached yet
     *
     ...
     *
     * Stores processed schema data to improve performance.
     *
     * @var array|null Array of schema data or null if not cached yet
     */
    ...
    {
        // Extract parameters from config array for backward compatibility.
        $collectionName = $name;
        ...
        try {
            ...
            try {
                ...
                $this->logger->debug('Document created for SOLR indexing', [
                    ...
                    'self_relations_type' => $selfRelationsType,
                    'self_relations_count' => $selfRelationsCount
                ]);
    ...
    public function bulkIndex(array $documents, bool $commit=false): bool
    {
        ...
        foreach ($documents as $i => $doc) {
            ...
            } elseif ($doc instanceof ObjectEntity) {
                $uuid = $doc->getUuid();
                if ($uuid === null) {
                    $uuid = 'null';
                }
    ...
    private function buildSolrQuery(array $query): array
    {
        ...
        if (!empty($query['_facets']) && $query['_facets'] !== 'extend') {
            ...
            foreach ($facets as $facetGroup => $facetConfig) {
                ...
                } elseif (is_array($facetConfig) === true && (($facetConfig['type'] ?? null) !== null)) {
    ...
    private function executeSearch(array $solrQuery, string $collectionName, array $_extend=[]): array
    {
        ...
        try {
            ...
            foreach ($solrQuery as $key => $value) {
                ...
                } elseif ($key === 'facet.field' && is_array($value) === true) {
    ...
    private function testSolrConnectivity(): array
    {
        try {
            ...
            } elseif (($data['responseHeader']['status'] ?? null) !== null && $data['responseHeader']['status'] === 0) {
                // System info response.
                $isValidResponse = true;
            } elseif (is_array($data) === true && empty($data) === false) {
    ...
    private function testSolrCollection(): array
    {
        try {
            ...
            } elseif (($this->solrConfig['core'] ?? null) !== null) {
    ...
    private function fetchSearchableObjects(\OCA\OpenRegister\Db\ObjectEntityMapper $objectMapper, int $limit, int $offset, array $schemaIds=[]): array
    {
        try {
            ...
            // Determine schema IDs filter for logging.
                    ...
                    try {
                        ...
                        if (str_contains($e->getMessage(), 'Schema is not searchable') === true) {
                            ...
                            $this->logger->warning('Unexpected non-searchable schema found despite pre-filtering', [
                                'objectId' => $objectEntity !== null ? $objectEntity->getId() : 'unknown',
                                'error' => $e->getMessage()
                            ]);
                            continue;
                        }
                        ...
                        $this->logger->warning('Failed to create SOLR document', [
                            'error' => $e->getMessage(),
                            'objectId' => is_array($object) === true ? ($object['id'] ?? 'unknown') : ($object instanceof ObjectEntity === true ? $object->getId() : 'unknown')
                        ]);
                    }
                }
    ...
    public function bulkIndexFromDatabaseParallel(int $batchSize = 1000, int $maxObjects = 0, int $parallelBatches = 4, array $solrFieldTypes = [], array $schemaIds = []): array
    {
        ...
        foreach ($batchChunks as $chunkIndex => $chunk) {
            ...
            // Calculate objectsPerSecond for logging.
            if ($totalTime > 0) {
                $objectsPerSecondValue = round(($totalIndexed / $totalTime) * 1000, 2);
            } else {
                $objectsPerSecondValue = 0;
            }
            ...
            $this->logger->info('Parallel bulk indexing completed', [
                ...
                'objectsPerSecond' => $objectsPerSecondValue
            ]);
    ...
    private function processBatchAsync($objectMapper, array $job): \React\Promise\PromiseInterface
    {
        return \React\Promise\resolve(null)->then(function() use ($objectMapper, $job) {
            ...
            if (empty($documents) === false) {
                ...
                $indexed = $success === true ? count($documents) : 0;
            }
    ...
    public function bulkIndexFromDatabaseHyperFast(int $batchSize = 5000, int $maxObjects = 10000): array
    {
        ...
        try {
            ...
            // Calculate objectsPerSecond for logging.
            if ($totalTime > 0) {
                $objectsPerSecondHyper = round(($totalIndexed / $totalTime) * 1000, 2);
            } else {
                $objectsPerSecondHyper = 0;
            }

            $this->logger->info('Hyper-fast bulk indexing completed', [
                'totalIndexed' => $totalIndexed,
                'totalTime' => $totalTime . 'ms',
                'objectsPerSecond' => $objectsPerSecondHyper
            ]);
    ...
    private function discoverFacetableFieldsFromSolr(): array
    {
        ...
        try {
            ...
            foreach ($schemaData['fields'] as $field) {
                ...
                } elseif (in_array($fieldName, ['_version_', 'id', '_text_']) === false) {
    ...
    private function discoverFieldsFromCurrentResults(array $solrResponse): array
    {
        ...
        foreach ($fieldsFound as $fieldName => $values) {
            ...
            } elseif (in_array($fieldName, ['_version_', 'id', '_text_'], true) === false) {
                // Object field (exclude system fields).
     ...
     *
     * @param array $_searchFacets Facets from current search results
     *
    ...
    /**
     * Get backend configuration (implements SearchBackendInterface).
     *
     * @return array Backend configuration array.
     */
    public function getConfig(): array
    {
        return $this->getSolrConfig();

    }//end getConfig()


    /**
     * Index generic documents (implements SearchBackendInterface).
     *
    ...
    {
        return $this->bulkIndex($documents, true);

    }//end index()


    /**
     * Perform a generic search query (implements SearchBackendInterface).
     *
    ...
    {
        return $this->searchObjects($params);

    }//end search()


    /**
     * Get field types for a collection (implements SearchBackendInterface).
     *
    ...
    {
        return $this->getSolrFieldTypes(true);

    }//end getFieldTypes()


    /**
     * Add a new field type to a collection (implements SearchBackendInterface).
     *
    ...
    {
        // Solr field types are managed via configSets, not dynamically.
        // For now, log and return success.
        $this->logger->info(
            'addFieldType called (not fully implemented for Solr)',
            [
                'collection' => $collection,
                'fieldType'  => $fieldType,
            ]
    ...
    /**
     * Get fields for a collection (implements SearchBackendInterface).
     *
    ...
    {
        $fieldsConfig = $this->getFieldsConfiguration();
    ...
    /**
     * Add or update a field in a collection (implements SearchBackendInterface).
     *
    ...
    {
        // For now, delegate to createMissingFields or log.
        $fieldName = $fieldConfig['name'] ?? 'unknown';
        $this->logger->info(
            'addOrUpdateField called',
            [
                'field' => $fieldName,
                'force' => $force,
            ]
        ...
        try {
            $result = $this->createMissingFields([$fieldName => $fieldConfig], false);
            if (isset($result['created']) && in_array($fieldName, $result['created'])) {
                return 'created';
            }
