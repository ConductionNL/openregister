...
class GuzzleSolrService
{
    ...
    /**
     * Create SOLR core (for standalone SOLR or as fallback)
     *
     ...
     * @throws \Exception When SOLR returns error response
     */
    public function createCore(string $coreName, string $configSetName): bool
    {
        $url = $this->buildSolrBaseUrl() . '/admin/cores?' . http_build_query([
            'action' => 'CREATE',
            'name' => $coreName,
            'configSet' => $configSetName,
            'wt' => 'json'
        ...
        if (($data['responseHeader']['status'] ?? -1) === 0) {
            $this->logger->info('SOLR core created successfully', [
                'core' => $coreName,
                'configSet' => $configSetName,
                'tenant_id' => $this->tenantId
            ]);
        ...
        $this->logger->error('SOLR core creation failed', [
            'core' => $coreName,
            'configSet' => $configSetName,
            'tenant_id' => $this->tenantId,
            'url' => $url,
        ...
        // Throw exception with SOLR response details
        throw new \Exception(
            "SOLR core creation failed: {$errorMessage}",
            $errorCode,
            new \Exception(json_encode($data))
        );
    }
