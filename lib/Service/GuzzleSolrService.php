...
class GuzzleSolrService
{
    ...
    public function deleteCollection(?string $collectionName=null): array
    {
        try {
            ...
            if ($this->collectionExists($targetCollection) === false) {
                return [
                    'success'    => false,
            ...
            $this->logger->info(
                    '🗑️ Attempting to delete SOLR collection',
                    [
                        'collection' => $targetCollection,
                        'url'        => $url,
                    ]
            ...
            $this->logger->error(
                    'Exception deleting SOLR collection',
                    [
    ...
    private function createLegacySolrDocument(ObjectEntity $object): array
    {
        ...
        if (empty($uuid) === true) {
            ...
            $uuid = $object->getId();
            $this->logger->warning(
                    'Object missing UUID - using object ID as fallback',
                    [
                        'object_id' => $object->getId(),
        ...
        $document = [
            ...
            '_text_' => $this->extractTextContent($object, $this->getObjectDataOrDefault($objectData)),
        ];
        ...
        if (is_array($objectData) === true) {
            foreach ($objectData as $key => $value) {
                if (is_array($value) === false && is_object($value) === false) {
                    ...
                    if (is_string($value) === true) {
                        // String field for faceting/filtering.
                        $document[$key.'_s'] = $value;
                        // Text field for full-text search.
                        $document[$key.'_t'] = $value;
        ...
        $document['self_object'] = json_encode($this->getObjectDataOrDefault($objectData));
        ...
        // DateTime fields.
        $document['self_created']     = $this->formatDateTimeField($object->getCreated());
        $document['self_updated']     = $this->formatDateTimeField($object->getUpdated());
        $document['self_published']   = $this->formatDateTimeField($object->getPublished());
        $document['self_depublished'] = $this->formatDateTimeField($object->getDepublished());

        // Complex fields as JSON.
        $document['self_authorization'] = $this->encodeJsonField($object->getAuthorization());
        $document['self_deleted']       = $this->encodeJsonField($object->getDeleted());
        $document['self_validation']    = $this->encodeJsonField($object->getValidation());
        $document['self_groups']        = $this->encodeJsonField($object->getGroups());
    ...
    private function translateOpenRegisterQuery(array $query): array
    {
        ...
        $this->logger->debug(
                'Building SOLR filters from query',
                [
                    'query_keys'      => array_keys($query),
                    'query_structure' => array_map(
                    function ($v) {
                        return $this->formatQueryStructureValue($v);
                    },
                    $query
                    ),
                ]
        ...
        foreach ($query as $key => $value) {
            ...
            if (is_array($value) === true) {
                ...
                if (isset($value['or']) === true) {
                    ...
                    $this->logger->debug(
                            'Filter with explicit AND logic',
                            [
                                'field'  => $key,
                                'values' => $values,
                            ]
                    ...
                    $this->logger->debug(
                            'Filter with default AND logic (array value)',
                            [
                                'field'  => $key,
                                'values' => $value,
                ...
                $this->logger->debug(
                        'Filter with AND logic (single value)',
                        [
                            'field' => $key,
                            'value' => $value,
        ...
        $this->logger->debug(
                'Final SOLR filter queries (each fq uses AND logic)',
                [
    ...
    }//end translateOpenRegisterQuery()
    ...
    private function convertSolrResultsToOpenRegisterFormat(array $solrResults, array $originalQuery): array
    {
        ...
        if (($solrResults['success'] ?? false) === false) {
            $this->logger->warning(
                    '[GuzzleSolrService] Search failed',
                    [
    ...
    }//end convertSolrResultsToOpenRegisterFormat()
    ...
    private function extractDynamicFields(array $objectData, string $prefix=''): array
    {
        ...
        foreach ($objectData as $key => $value) {
            ...
            $basePrefix = $this->getBasePrefix($prefix);
    ...
    public function bulkIndex(array $documents, bool $commit=false): bool
    {
        ...
        try {
            ...
            if ($solrStatus !== 0) {
                ...
                throw new \RuntimeException(
                    "SOLR bulk index failed: SOLR status {$solrStatus}. "."Error: {$errorDetails['error_msg']} (Code: {$errorDetails['error_code']}). "."HTTP Status: {$statusCode}",
                    $solrStatus
                );
            }
    ...
    private function countSearchableObjects(\OCA\OpenRegister\Db\ObjectEntityMapper $objectMapper, array $schemaIds=[]): int
    {
        try {
            ...
            $this->logger->warning(
                    'Failed to count searchable objects, falling back to all objects',
                    [
                        'error' => $e->getMessage(),
                    ]
                    );
    ...
    public function bulkIndexFromDatabase(int $batchSize=1000, int $maxObjects=0, array $solrFieldTypes=[], array $schemaIds=[]): array
    {
        ...
        try {
            ...
            $this->logger->info(
                    '📊 Sequential bulk index planning (searchable objects only)',
                    [
                ...
                foreach ($objects as $object) {
                    try {
                        ...
                        if (str_contains($e->getMessage(), 'Schema is not searchable') === true) {
                            ...
                            $this->logger->warning(
                                    'Unexpected non-searchable schema found despite pre-filtering',
                                    [
                                        'objectId' => $objectId,
                                        'error'    => $e->getMessage(),
                                    ]
            ...
            $this->logger->info(
                    'Sequential bulk indexing completed',
                    [
            ...
            throw new \RuntimeException(
                'Serial bulk indexing failed: '.$e->getMessage().' (Indexed: '.$totalIndexedValue.', Batches: '.$batchCountValue.')',
                0,
                $e
            );
        }//end try
     ...
     * @param  array $solrFieldTypes  Optional array of SOLR field types
     * @param  array $schemaIds       Optional array of schema IDs to filter by
    ...
    public function bulkIndexFromDatabaseParallel(int $batchSize=1000, int $maxObjects=0, int $parallelBatches=4, array $solrFieldTypes=[], array $schemaIds=[]): array
    {
        ...
        try {
            ...
            if ($totalTime > 0) {
                $objectsPerSecond = round(($totalIndexed / $totalTime) * 1000, 2);
            } else {
                $objectsPerSecond = 0;
            }

            $this->logger->info(
                'Parallel bulk indexing completed',
                [
                    'totalIndexed'     => $totalIndexed,
            ...
            throw new \RuntimeException(
                'Parallel bulk indexing failed: '.$e->getMessage().' (Indexed: '.$totalIndexedValue.', Batches: '.$totalBatchesValue.')',
                0,
                $e
            );
        }//end try

    }//end bulkIndexFromDatabaseParallel()
    ...
    private function processBatchDirectly($objectMapper, array $job, array $schemaIds=[]): array
    {
        ...
        try {
            ...
            if (empty($documents) === false) {
                $this->logger->info(
                        '🔥 WARMUP: About to bulk index batch',
                        [
                ...
                $bulkResult = $this->bulkIndex($documents, true);
                // Commit each batch for immediate visibility
                $this->logger->info(
                ...
                if ($bulkResult === true) {
                    ...
                    $this->logger->error(
                            '🔥 WARMUP: Bulk index returned FALSE!',
                            [
                                'batchNumber'   => $job['batchNumber'],
                                'documentCount' => count($documents),
                            ]
                ...
                $this->logger->warning(
                        '🔥 WARMUP: No documents to index in batch',
                        [
                            'batchNumber' => $job['batchNumber'],
                            'offset'      => $job['offset'],
                            'limit'       => $job['limit'],
    ...
    {
        return \React\Promise\resolve(null)->then(
                function () use ($objectMapper, $job) {
                    $batchStartTime = microtime(true);

                    // Fetch objects directly from ObjectEntityMapper.
                    $this->logger->debug(
                    'Processing batch async with ObjectEntityMapper',
                    [
                        'batchNumber' => $job['batchNumber'],
                        'offset'      => $job['offset'],
                        'limit'       => $job['limit'],
                    ...
                    $indexed = 0;
                    if (empty($documents) === false) {
                        $success = $this->bulkIndex($documents, true);
                        // Commit each batch for immediate visibility
                        if ($success === true) {
                            $indexed = count($documents);
                        } else {
    ...
    public function bulkIndexFromDatabaseHyperFast(int $batchSize=5000, int $maxObjects=10000): array
    {
        ...
        try {
            ...
            $startTime = microtime(true);
            $this->logger->info(
                    'Starting hyper-fast bulk index using ObjectEntityMapper',
                    [
                        'batchSize'  => $batchSize,
                        'maxObjects' => $maxObjects,
                    ]
            ...
            $this->logger->info(
                    'Fetching all objects in single batch',
                    [
                        'limit' => $maxObjects,
                    ]
                    );
            ...
            $this->logger->info(
                    'Prepared documents for bulk index',
                    [
                        'documentCount' => count($documents),
                    ]
                    );
            ...
            $this->logger->info(
                    'Hyper-fast bulk indexing completed',
                    [
                        'totalIndexed'     => $totalIndexed,
                        'totalTime'        => $totalTime.'ms',
                        'objectsPerSecond' => $objectsPerSecond,
            ...
            throw new \RuntimeException(
                'Hyper-fast bulk indexing failed: '.$e->getMessage().' (Indexed: '.$totalIndexedValue.', Batches: '.$batchCountValue.')',
                0,
                $e
            );
        }//end try

    }//end bulkIndexFromDatabaseHyperFast()
     ...
     *
     * @param  bool $forceRefresh Whether to force refresh of cached field types
    ...
    private function getSolrFieldTypes(bool $forceRefresh=false): array
    {
        ...
        if ($forceRefresh === false && $this->cachedSolrFieldTypes !== null) {
            $this->logger->debug(
                    '🚀 Using cached SOLR field types',
                    [
        ...
        try {
            ...
            $this->logger->info(
                    '🔍 Retrieved and cached SOLR field types for validation',
                    [
                        'field_count'       => count($fieldTypes),
                        'sample_fields'     => array_slice($fieldTypes, 0, 5, true),
                        'versie_field_type' => $versieFieldType,
                    ]
                    );
            ...
            $this->logger->warning(
                    'Failed to retrieve SOLR field types',
                    [
                        'error' => $e->getMessage(),
                    ]
                    );
    ...
    public function warmupIndex(array $schemas=[], int $maxObjects=0, string $mode='serial', bool $collectErrors=false, int $batchSize=1000, array $schemaIds=[]): array
    {
        ...
        try {
            ...
            if ($mode === 'hyper') {
                $indexResult = $this->bulkIndexFromDatabaseOptimized($batchSize, $maxObjects, $solrFieldTypesValue, $schemaIds);
            } else if ($mode === 'parallel') {
                $indexResult = $this->bulkIndexFromDatabaseParallel($batchSize, $maxObjects, 5, $solrFieldTypesValue, $schemaIds);
            } else {
                $indexResult = $this->bulkIndexFromDatabase($batchSize, $maxObjects, $solrFieldTypesValue, $schemaIds);
            }
            ...
            $memoryReport = $this->generateMemoryReport($initialMemoryUsage, $finalMemoryUsage, $initialMemoryPeak, $finalMemoryPeak, $memoryPredictionValue);
    ...
    public function bulkIndexFromDatabaseOptimized(int $batchSize=1000, int $maxObjects=0, array $solrFieldTypes=[], array $schemaIds=[]): array
    {
        ...
        try {
            ...
            $totalObjects = $this->countSearchableObjects($objectMapper, $schemaIds);
            if ($maxObjects > 0) {
                $actualLimit = min($maxObjects, $totalObjects);
            } else {
                $actualLimit = $totalObjects;
            }

            if (empty($solrFieldTypes) === false) {
                $fieldTypesCached = true;
            } else {
                $fieldTypesCached = false;
            }

            $this->logger->info(
                    '🚀 Starting optimized bulk indexing',
                    [
                        'total_objects'      => $totalObjects,
                        'actual_limit'       => $actualLimit,
                        'batch_size'         => $batchSize,
                        'field_types_cached' => $fieldTypesCached,
                    ]
                    );
            ...
            for ($offset = 0; $offset < $actualLimit; $offset += $batchSize) {
                ...
                if (empty($documents) === false) {
                    $indexResult = $this->bulkIndex($documents, true);
                    // Commit each batch for immediate visibility.
                    if ($indexResult === true) {
                        $totalIndexed += count($documents);
                    } else {
            ...
            $this->logger->error(
                    'Optimized bulk indexing failed',
                    [
                        'error'          => $e->getMessage(),
    ...
    public function deleteField(string $fieldName): array
    {
        ...
        try {
            ...
            if ($success === true) {
                $this->logger->info(
                        '✅ SOLR field deleted successfully',
                        [
                            'field_name' => $fieldName,
                            'collection' => $collectionName,
                        ]
    ...
    private function getExtendedFacetData(array $facetableFields, array $filters=[]): array
    {
        ...
        $this->logger->debug(
                'Built JSON facet query',
                [
                    'facetQueryKeys'   => array_keys($facetQuery),
                    'objectFacetKeys'  => array_filter(
                    array_keys($facetQuery),
                    function ($key) {
        ...
        try {
            ...
            if ($data === null) {
                $this->logger->error(
                        'Failed to decode SOLR JSON response',
                        [
    ...
    public function indexFileChunks(int $fileId, array $chunks, array $metadata): bool
    {
        try {
            ...
            $this->logger->info(
                    '[GuzzleSolrService] Indexing file chunks',
                    [
                        'file_id'     => $fileId,
            ...
            $this->logger->error(
                    '[GuzzleSolrService] Failed to index file chunks',
                    [
                        'file_id'  => $fileId,
                        'response' => $result,
                    ]
            ...
            $this->logger->error(
                    '[GuzzleSolrService] Exception indexing file chunks',
                    [
                        'file_id' => $fileId,
                        'error'   => $e->getMessage(),
                    ]
     ...
     * @param int   $processedObjects Processed objects count.
     *
     * @return float Objects per second.
     */
    private function calculateObjectsPerSecond(float $durationSeconds, int $processedObjects): float
    {
        if ($durationSeconds > 0) {
            return round($processedObjects / $durationSeconds, 2);
        }
