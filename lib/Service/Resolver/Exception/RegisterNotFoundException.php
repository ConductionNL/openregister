...
/**
 * OpenRegister Resolver RegisterNotFoundException
 *
 * Exception thrown when a configured register slug/UUID cannot be resolved.
 *
 * @category Exception
...
/**
 * Thrown when a register config key is set but the resolved slug/UUID matches no entity.
 *
 * Two distinct failure modes surface via this exception:
 * - The register simply does not exist anywhere (stale config, HTTP 500 hint).
 * - The register exists but not in the caller's tenant (HTTP 404 hint).
 *
 ...
 * @link https://OpenRegister.app
 */
class RegisterNotFoundException extends OpenRegisterException
{
    ...
    public function __construct(
        ...
        parent::__construct(
            message: "Register '{$resolvedValue}' (config key '{$configKey}' in app '{$appId}') could not be found.",
            code: 404,
            previous: $previous
        );
    }//end __construct()
