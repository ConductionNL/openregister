...
class ObjectService
{
    ...
    public function __construct(
        ...
        $this->logger->debug('[ObjectService] ObjectService constructor completed.');
    }//end __construct()
            ...
            if (is_array($extend) === true && in_array('_names', $extend, true) === true) {
                ...
                if (is_array($resultsToProcess) === false || empty($resultsToProcess) === true) {
                    ...
                    try {
                        ...
                        $this->logger->error('[ObjectService] _names extension failed: '.$e->getMessage().' at '.$e->getFile().':'.$e->getLine());
                        $result['@self']['names'] = [];
