...
class SolrFileService
{
     ...
     */
    private const DEFAULT_CHUNK_SIZE = 1000;      // tokens
    ...
    public function processAndIndexFile(string $filePath, array $metadata): array
    {
        ...
        $this->logger->info('Processing file for indexing', [
            'file' => $filePath,
    ...
    private function extractFromPdf(string $filePath): string
    {
        ...
        if (class_exists('\Smalot\PdfParser\Parser')) {
            try {
                ...
                $this->logger->warning('Smalot PDF parser failed, trying pdftotext', [
                    'error' => $e->getMessage()
                ]);
            }
        }
    ...
    public function chunkDocument(string $text, array $options = []): array
    {
        ...
        if (count($chunks) > self::MAX_CHUNKS_PER_FILE) {
            $this->logger->warning('File exceeds max chunks, truncating', [
                'chunks' => count($chunks),
    ...
    public function indexFileChunks(string $fileId, array $chunks, array $metadata): array
    {
        ...
        $this->logger->info('Indexing file chunks to fileCollection', [
            'file_id' => $fileId,
            'chunk_count' => count($chunks),
            'collection' => $collection
        ]);
    ...
    public function searchFiles(array $query = []): array
    {
        ...
        $this->logger->debug('Searching files in fileCollection', [
            'collection' => $collection,
            'query' => $query
        ]);
    ...
    public function deleteFile(string $fileId): bool
    {
        ...
        $this->logger->info('Deleting file from fileCollection', [
            'file_id' => $fileId,
            'collection' => $collection
        ]);
    ...
    public function processExtractedFiles(?int $limit = null, array $options = []): array
    {
        ...
        $this->logger->info('Starting bulk file chunking and indexing', [
            'limit' => $limit,
            'collection' => $collection
        ]);
        ...
        foreach ($fileTexts as $fileText) {
            try {
                ...
                if (empty($text)) {
                    $this->logger->warning('Empty text content for file', [
                ...
                // Chunk the text
                $chunks = $this->chunkDocument($text, array_merge($options, [
                    'file_type' => $fileText->getMimeType()
                ]));

                if (empty($chunks)) {
                    $this->logger->warning('No chunks produced for file', [
                        'file_id' => $fileText->getFileId()
                    ]);
                    continue;
                }
                ...
                if ($result['success']) {
                    ...
                    $this->logger->info('Successfully indexed file chunks', [
                        'file_id' => $fileText->getFileId(),
                ...
                $this->logger->error('Failed to process file for chunking', [
                    'file_id' => $fileText->getFileId(),
                    'error' => $e->getMessage()
                ]);
            }
        ...
        $this->logger->info('Completed bulk file chunking', [
            'stats' => $stats
        ]);
    ...
    public function processExtractedFile(int $fileId, array $options = []): array
    {
        ...
        $this->logger->info('Processing single extracted file', [
            'file_id' => $fileId,
            'collection' => $collection
        ]);
