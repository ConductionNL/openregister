...
class Version1Date20251116000000 extends SimpleMigrationStep
{
    ...
    private function createObjectTextTable(IOutput $output, ISchemaWrapper $schema): void
    {
        if ($schema->hasTable('openregister_object_texts') === true) {
            $output->info('ℹ️  Table openregister_object_texts already exists, skipping.');
            return;
        }
        ...
        $table->addColumn('register', Types::STRING, [
            'length' => 255,
            'notnull' => true,
        ]);
        $table->addColumn('schema', Types::STRING, [
            'length' => 255,
            'notnull' => true,
        ]);
        ...
        $table->addColumn('text_length', Types::INTEGER, [
            'notnull' => true,
            'default' => 0,
        ]);
        $table->addColumn('property_map', Types::JSON, [
            'notnull' => false,
        ]);
        $table->addColumn('extraction_status', Types::STRING, [
            'length' => 32,
            'default' => 'completed',
            'notnull' => true,
        ]);
        $table->addColumn('chunked', Types::BOOLEAN, [
            'default' => false,
            'notnull' => true,
        ]);
        ...
        $output->info('✅ Created openregister_object_texts table.');
    }
    ...
    private function createChunksTable(IOutput $output, ISchemaWrapper $schema): void
    {
        if ($schema->hasTable('openregister_chunks') === true) {
            $output->info('ℹ️  Table openregister_chunks already exists, skipping.');
            return;
        }
        ...
        $table->addColumn('position_reference', Types::JSON, [
            'notnull' => false,
        ]);
        ...
        $table->addColumn('language_level', Types::STRING, [
            'length' => 20,
            'notnull' => false,
        ]);
        $table->addColumn('language_confidence', Types::DECIMAL, [
            'precision' => 3,
            'scale' => 2,
            'default' => 0,
            'notnull' => false,
        ]);
        $table->addColumn('detection_method', Types::STRING, [
            'length' => 50,
            'notnull' => false,
        ]);
        $table->addColumn('indexed', Types::BOOLEAN, [
            'default' => false,
            'notnull' => true,
        ]);
        $table->addColumn('embedding_provider', Types::STRING, [
            'length' => 100,
            'notnull' => false,
        ]);
        $table->addColumn('overlap_size', Types::INTEGER, [
            'default' => 0,
            'notnull' => true,
        ]);
        $table->addColumn('vectorized', Types::BOOLEAN, [
            'default' => false,
            'notnull' => true,
        ]);
        $table->addColumn('owner', Types::STRING, [
        ...
        $output->info('✅ Created openregister_chunks table.');
    }
    ...
    private function createEntitiesTable(IOutput $output, ISchemaWrapper $schema): void
    {
        if ($schema->hasTable('openregister_entities') === true) {
            $output->info('ℹ️  Table openregister_entities already exists, skipping.');
            return;
        }
        ...
        $table->addColumn('belongs_to_entity_id', Types::BIGINT, [
            'notnull' => false,
        ]);
        ...
        $table->addColumn('detected_at', Types::DATETIME, [
            'notnull' => true,
        ]);
        $table->addColumn('updated_at', Types::DATETIME, [
            'notnull' => true,
        ...
        $output->info('✅ Created openregister_entities table.');
    }
    ...
    private function createEntityRelationsTable(IOutput $output, ISchemaWrapper $schema): void
    {
        if ($schema->hasTable('openregister_entity_relations') === true) {
            $output->info('ℹ️  Table openregister_entity_relations already exists, skipping.');
            return;
        }
        ...
        $table->addColumn('email_id', Types::BIGINT, [
            'notnull' => false,
        ]);
        $table->addColumn('position_start', Types::INTEGER, [
            'notnull' => true,
            'default' => 0,
        ]);
        $table->addColumn('position_end', Types::INTEGER, [
            'notnull' => true,
            'default' => 0,
        ]);
        ...
        $table->addColumn('context', Types::TEXT, [
            'notnull' => false,
        ]);
        $table->addColumn('anonymized', Types::BOOLEAN, [
            'default' => false,
            'notnull' => true,
        ]);
        $table->addColumn('anonymized_value', Types::STRING, [
            'length' => 255,
            'notnull' => false,
        ]);
        $table->addColumn('created_at', Types::DATETIME, [
