...
class Version002003000Date20251013000000 extends SimpleMigrationStep
{
    ...
    public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper
    {
        ...
        if (!$schema->hasTable('openregister_vectors')) {
            ...
            $table->addColumn('chunk_index', 'integer', [
                'notnull' => true,
                'default' => 0,
                'comment' => '0 for objects, N for file chunks'
            ]);

            $table->addColumn('total_chunks', 'integer', [
                'notnull' => true,
                'default' => 1,
                'comment' => '1 for objects, N for files'
            ]);

            $table->addColumn('chunk_text', 'text', [
                'notnull' => false,
            ...
            $table->addColumn('embedding_model', 'string', [
                'notnull' => true,
                'length' => 100,
                'comment' => 'Model used to generate embeddings (e.g., text-embedding-ada-002)'
            ]);

            $table->addColumn('embedding_dimensions', 'integer', [
                'notnull' => true,
