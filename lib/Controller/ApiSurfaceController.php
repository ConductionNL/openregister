...
class ApiSurfaceController extends Controller {
	 ...
	 * @contract tests/Unit/Controller/ApiSurfaceControllerTest.php
	 *
	 * @no-admin-idor-exempt The {version} segment names an instance-wide contract
	 * declaration, not an object anybody owns. Every caller receives the same
	 * document for the same version, the catalogue holds no per-user and no
	 * per-tenant rows, and the route constrains the value to one to three digits.
	 * There is nothing here to scope to the caller: the underlying description is
	 * generated by OasService from the registers and schemas, and the two routes
	 * that already publish exactly that document (`oas#generate` and
	 * `oas#generateAll`) are `@PublicPage`. This one keeps a session requirement
	 * as the narrower posture of the two, not because an object needs scoping.
	 */
	#[NoAdminRequired]
	#[NoCSRFRequired]
