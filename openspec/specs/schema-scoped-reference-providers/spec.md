...
@e2e exclude backend Smart Picker / unified-search provider base classes — covered by PHPUnit

Lets a consuming Nextcloud app expose a single OpenRegister (register, schema) pair as its own named entry in the Smart Picker and unified search, by subclassing two reusable abstract base classes instead of hand-rolling URL parsing, RBAC-safe search, and rich-preview formatting. Complements the existing single, fleet-wide `openregister_objects` / `openregister-ref-objects` providers (see `mail-smart-picker`, `zoeken-filteren`) rather than replacing them: a schema with no schema-scoped subclass continues to be found only through the generic providers. The provider id, title, and icon are computed deterministically by OpenRegister from the schema's own identity and metadata — never chosen by the consuming app — and participation is gated by a `smartPickerEnabled` schema flag mirroring the existing `searchable` flag.

**Status**: done

**OpenSpec changes**
- [`schema-scoped-smart-picker`](../../changes/archive/2026-08-17-schema-scoped-smart-picker/) _(archived 2026-08-17)_ — implements `AbstractSchemaReferenceProvider` and `AbstractSchemaSearchProvider`, the `Schema::smartPickerEnabled` flag and its migration, the shared `ObjectPreviewFormatter` / `ObjectSearchResultFormatter` extraction from the generic providers, `IPublicReferenceProvider` support on `ObjectReferenceProvider`, and reference-cache invalidation on object save.
...
## Cross-References
- `mail-smart-picker` — the generic `ObjectReferenceProvider` this change's abstract base classes reuse and delegate to.
- `zoeken-filteren` / `unified-search-provider` — the generic `ObjectsProvider` this change's abstract base classes reuse and delegate to.
- `deep-link-registry` — URL/icon resolution consulted by the shared formatting services both the generic and schema-scoped providers use.
