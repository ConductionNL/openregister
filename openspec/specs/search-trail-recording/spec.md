...
OpenRegister records a *search trail* — the term searched, the result and total counts, the response time, and the execution backend — for paginated searches so that the dashboard's "Popular Search Terms" widget and "Searches" KPI have data to display. The read side (`SearchTrailService`, `SearchTrailMapper`, popular-terms and statistics aggregations) already existed; this capability supplies the write path that feeds it.

Recording is governed by an admin-configurable retention setting `searchTrailRecordingMode` (`all` | `_search` | `none`, default `_search`), combined with the existing `searchTrailsEnabled` master switch into an effective mode. Recording is best-effort: a write failure logs a warning and never degrades search availability.

**Standards**: BIO (audit logging), AVG/GDPR Article 30 (processing register context).
**Cross-references**:
- **zoeken-filteren** — the paginated search path (`ObjectService::searchObjectsPaginated()`) where recording is invoked, covering both the SOLR/index and database backends.
- **search-index** — `IndexService::searchObjectsPaginated()` is one of the recorded backends (execution type identifies it).

**OpenSpec changes**: [search-trail-recording](../../changes/archive/2026-06-22-search-trail-recording/) _(archived 2026-06-22)_
