...
**OpenSpec changes**
- [rbac-as-public-toggle](../../changes/archive/2026-08-25-rbac-as-public-toggle/) _(archived 2026-08-25)_ — adds the `_rbac_as_public` query-flag primitive that forces an anonymous RBAC context regardless of the caller's session. Enables OpenCatalogi's WOO-536 architecture fix (Robert Zondervan, 2026-08-12) by giving public endpoints a way to preserve session-independent visibility while still consuming OR's schema-level RBAC (rather than the current PHP post-filter). Ships as OR precursor PR #2855 before the OpenCatalogi consumer change.
...
SCH-PFTS-001 (OpenCatalogi public search endpoint) mandates uniform visibility. Phase 1 discovery of WOO-536 confirmed no existing escape hatch existed, making this small OR precursor necessary.
