...
- **Partially implemented -- webhook retry**: `WebhookRetryJob` (`lib/BackgroundJob/WebhookRetryJob.php`) and `WebhookDeliveryJob` (`lib/BackgroundJob/WebhookDeliveryJob.php`) handle async delivery and retry with configurable policies (exponential, linear, fixed backoff).
