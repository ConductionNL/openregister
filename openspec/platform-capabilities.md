...
| `x-openregister-aggregations` | implemented (pilot, decidesk ActionItem) | `aggregations-annotation` change | Declarative `count`/`sum`/`avg`/`min`/`max`/`count_distinct` queries with optional groupBy. `GET /api/objects/aggregations/{register}/{schema}/{name}`. PHP-side runner over MagicMapper; backend-native aggregation (Postgres GROUP BY / Solr facets / ES aggs) is a follow-up. Stored under `configuration['x-openregister-aggregations']`. |
| `x-openregister-calculations` | implemented (pilot, decidesk ActionItem) | `calculations-annotation` change | Computed fields. JSON-shaped expression AST (concat, if, arithmetic, comparison, date diff, placeholders). `materialise: true` evaluates at save and persists; can be targeted by aggregations. `materialise: false` (virtual, on-read) is a follow-up. Stored under `configuration['x-openregister-calculations']`. |
| `x-openregister-notifications` | implemented v1 (pilot, decidesk Meeting) | `notifications-annotation` change | Declarative notifications via `INotificationManager`. v1 supports trigger types `created`/`updated`/`transition` (with optional action filter), recipient kinds `users` (literal list) / `field` (uid from object), channel `nc-notification`. Subject template supports `{{prop}}` interpolation. Webhook auto-create, scheduled, threshold, email/Talk/Activity channels deferred. Stored under `configuration['x-openregister-notifications']`. |
