...
| `x-openregister-aggregations` | proposed | `aggregations-annotation` change | Declarative `count`/`sum`/`avg`/`min`/`max`/`count_distinct` queries with optional groupBy + time-bucket. `GET /api/objects/aggregations/{name}`. RBAC-scoped, cached, invalidates on writes. |
| `x-openregister-calculations` | proposed | `calculations-annotation` change | Computed fields. Expression DSL over other properties (concat, if, arithmetic, comparison, date diff). Materialise at save (aggregatable, persisted) or compute on read (virtual). Aggregations MAY target materialised calculation fields. |
