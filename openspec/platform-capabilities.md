...
| Chat/agents API deprecation + optional hermiq proxy | implemented | `chat-ai` (`or-chat-proxy-deprecation` change) | `ChatCompatMiddleware` + `Service\Chat\ChatProxyHandler`. Every `Chat*`/`Conversation`/`Agents` API response carries `Deprecation`/`Sunset`/`Link: rel="successor-version"` headers now that hermiq owns its own copy of the engine (`hermiq#12`/`#13`, merged). Optional, off-by-default `openregister.chat.proxyTo=hermiq` appconfig forwards JSON calls server-side and 308-redirects the SSE stream endpoint to hermiq's mirrored routes; falls back to local serving on any failure. OR's own engine/tables/routes are NOT deleted — that is a separate future change. |
