...
> **Status:** Core (5 of 16) implemented in `SaveObject::validateReferences()` and verified by `tests/Service/ReferenceExistenceValidationIntegrationTest`. The remaining 11 are extensions — batch optimisation, structured errors, circular detection, GraphQL/async/events/strictness — left open as separate work.
...
- [ ] Validation error reporting includes structured diagnostic information. Today the error message is a string. **Open** — should expose `propertyName`, `referencedUuid`, `targetSchemaSlug`, `targetRegister` as structured fields on the exception.
...
- [x] `tests/Service/ReferenceExistenceValidationIntegrationTest` — 5 tests against real schema + Postgres:
