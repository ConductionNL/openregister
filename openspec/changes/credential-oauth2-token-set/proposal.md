...
- Catalogue entries for LinkedIn, Meta Graph, X, Bluesky, Mastodon and Google, each with its authorization, token and revoke endpoints, its refresh grant shape, and allow-rules covering publishing a post, reading own post metrics, reading followers and querying Search Console. Google is two entries, Search Console and Analytics, because the host-lock is per host and the two APIs answer on different hosts, exactly as the live and test Adyen entries are already split.
