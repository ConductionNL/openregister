...
- [ ] 3.1 Replace the inline schema-expansion block (`lib/Controller/RegistersController.php:286-345`) with a single call to `$this->registerService->findAllSerialized(...)`, passing the parsed `$_extend` array.
...
- [ ] 7.3 Manually verify (in a local dev environment) that DocuDesk's admin settings schema dropdown remains empty on the current OpenRegister build after this change — confirming no accidental fix-through-magic. The DocuDesk dropdown populates only after DocuDesk's own follow-up PR.
