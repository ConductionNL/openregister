...
- Support the `_extend` values currently recognized by the controller:
  - `schemas` — expand schema IDs into full schema objects (with `properties` filtered, matching current controller behavior).
...
No breaking changes to the HTTP API. No breaking changes to `Register::jsonSerialize()`. The change is additive at the service layer.
...
- `lib/Controller/RegistersController.php` — `index()` delegates expansion to the serializer; existing inline expansion block (≈ lines 286–345) is removed.
