...
- [~] 6.1 In `ChatStreamController`, track `$lastEventAt = microtime(true)` and update it after every emitted event — **Deferred** — see 6.3 rationale below. — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] 6.2 In the tool-loop and LLM streaming callback, check `microtime(true) - $lastEventAt >= 15.0` before each yield point; if true, emit `heartbeat: {"ts": "<ISO-8601>"}` and reset `$lastEventAt` — **Deferred.** — deferred to downstream cycle / fleet-wide adoption (handoff)
- [~] 6.3 Validate in `ChatStreamControllerTest` that a mock LLM taking 35s (simulated via mock) triggers at least two heartbeat events — **Deferred.** — deferred to downstream cycle / fleet-wide adoption (handoff)
