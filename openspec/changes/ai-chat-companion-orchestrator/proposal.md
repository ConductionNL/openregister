...
- **NO BREAKING CHANGES.** Existing `POST /api/chat/send` non-streaming endpoint unchanged. Existing full-page chat at `/apps/openregister/chat` (`src/views/chat/ChatIndex.vue`) unchanged in this change — refactoring it onto the shared nc-vue primitives is tracked separately in [openregister#1459](https://codeberg.org/Conduction/openregister/issues/1459).
