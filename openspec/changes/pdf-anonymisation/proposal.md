...
1. **SAPP fork dependency.** SAPP today supports only `/FlateDecode`. To handle the filter / encoding variety in real-world Woo PDFs we need to extend SAPP with: four additional stream-filter decoders (LZW, ASCII85, ASCIIHex, RunLength), a ToUnicode CMap parser + font-encoding resolver, a TJ-kerning-array flattening pre-pass, and a flagship `replaceTextInDocument()` API with font-switch + base-font-fallback support. We're upstreaming all of this as a series of small PRs to `dealfonso/sapp` (LGPL-3.0-or-later). During development OpenRegister consumes from `ConductionNL/sapp` `work/text-replacement` via a VCS composer repository; when upstream merges, we point back at upstream tags. See `upstream-issues/` for the planned PR series.
...
- **NEW dependency** on a fork: `ddn/sapp:dev-work/text-replacement` from `ConductionNL/sapp`. Until SAPP upstream merges the work, composer pins to the fork branch via a VCS repository. Tracked under `tasks.md` §10.
...
- **Affected code:**
  ...
  - `composer.json` — add VCS repository entry for `ConductionNL/sapp` + `ddn/sapp:dev-work/text-replacement as 1.x-dev`.
...
- **Operational dependency:** `ConductionNL/sapp` fork must be reachable from CI for composer install. Until upstream merges, the fork branch is the source of truth.
