...
Defines the PDF branch of `DocumentProcessingHandler::anonymizeDocument`. Today PDFs fall through to a binary `str_ireplace` path that corrupts the output. This capability extends the dispatch with a SAPP-driven byte-level replacement pipeline that satisfies five hard constraints — no original PII in the output, identifiable placeholders, layout preservation, FOSS-only dependencies, no operational sidecars — none of which the alternatives (visual overlay, render-from-text, commercial libraries, language-sidecar services, LibreOffice CLI, flatten-and-OCR) collectively satisfy. The discovery doc (`pdf-anonymisation-discovery`) walks through the eight-option matrix; this spec locks the chosen path.
