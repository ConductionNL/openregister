...
## Purpose
Provide a reusable, additive OpenRegister foundation surface that a leaf app
(procest "cases on map", issue #112) consumes: a page-level map that plots many
objects of a register/schema as markers on one map. The point query is
RBAC-scoped and fail-closed. OpenRegister owns the render contract; the leaf
owns point selection. The per-object `MapsProvider` sidebar surface is unchanged.

**Standards**: ADR-005 (authorization / fail-closed), ADR-019 (pluggable
integration registry), ADR-022 (apps consume OR abstractions).
**Cross-references**: `integration-maps` (per-object sidebar),
`integration-leaf-foundation-shares-analytics` (page-widget pattern), `geo-metadata-kaart`.
