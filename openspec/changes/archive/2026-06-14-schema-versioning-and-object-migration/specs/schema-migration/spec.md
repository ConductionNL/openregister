...
Manage schema evolution over live object populations: classify every
schema definition change as compatible or breaking and keep a
per-version changelog; re-validate existing objects against the current
or a proposed definition (impact analysis) without mutating data; track
which schema version each object last validated against and expose a
queryable validity status; execute declarative, batched, auditable
migration runs through the standard save pipeline; roll a run back via
content-versioning; and gate breaking changes (including those arriving
through the runtime schema API) behind explicit acknowledgement.
