...
- [x] 7.1 Walk all ~31 routes in browser-1; confirm each renders, matches the
  ...
  > IDs (`/api/audit-trails/statistics`, `/api/applications/1`,
  > `/api/entities/1`, `/api/objects/1/1/00000000-...`); none originate in OR
  > shell logic. NO ADR-036 `customComponents` deprecation warning fires —
  > superseded by the post-change registry-prop migration (PR #1988).
