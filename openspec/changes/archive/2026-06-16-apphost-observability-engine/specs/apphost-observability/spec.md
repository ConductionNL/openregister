...
# AppHost: Declarative Observability

## Purpose

Execute health checks and Prometheus metrics declared as JSON in a host app's `src/manifest.json` `observability` block, so leaf apps ship zero hand-written observability PHP while keeping their existing `/apps/{appid}/api/health` and `/apps/{appid}/api/metrics` URLs and the ADR-006 contract.

**Standards**: ADR-006 (observability endpoints), ADR-040 (declarative observability block), ADR-035 (provider alias discovery), Prometheus text exposition 0.0.4
**Cross-references**: `apphost-boilerplate-controllers` (alias bootstrap), per-app `adopt-apphost` changes
