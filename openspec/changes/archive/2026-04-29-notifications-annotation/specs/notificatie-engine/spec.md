...
> **Superseded — graduated into the canonical spec.**
>
> The normative MUSTs below — channel-block format, throttle-window
> grammar, `x-openregister-notifications` annotation contract — have
> been merged into `openspec/specs/notificatie-engine/spec.md` via the
> `lifecycle-notifications-amendments` change (2026-06-10). This file
> remains as the design record; the canonical spec is the single
> source of truth. Do not edit this file to refine the notifications
> contract — open a new change against the canonical path instead.
