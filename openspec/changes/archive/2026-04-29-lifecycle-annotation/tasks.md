...
- [ ] 1.2 Create `lib/Listener/LifecycleValidationListener.php` — subscribes to `ObjectUpdatingEvent`; if the schema declares `x-openregister-lifecycle` and the new `field` value isn't in `transitions[*].to` reachable from the current value, call `event->stopPropagation()` + `event->setErrors([...])` (reusing the existing `setErrors`/`getErrors` API on `ObjectUpdatingEvent`).
- [ ] 1.2a Verify `ObjectService::saveObject` translates a stopped `ObjectUpdatingEvent` with non-empty `getErrors()` into HTTP 422 with the errors array as the response body. The shipped `ObjectUpdatingEvent` already exposes `setErrors(array $errors): void` / `getErrors(): array` — DO NOT introduce new `setRejectionReason` / `getRejectionReason` names (renaming the publicly-exposed event API is a breaking change for third-party listeners).
