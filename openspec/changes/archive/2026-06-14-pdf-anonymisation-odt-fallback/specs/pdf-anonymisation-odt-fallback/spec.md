...
Reserves the change id for the discovery doc's Path B (NC Office ODT round-trip) fall-through to the v1 `pdf-anonymisation` change. The orchestrator seam ships in this change as a **dormant scaffold** — the wiring exists in production code but defaults to inert (`pdf-anonymisation.path-b-enabled = false` + `NullNcOfficeConverter` DI binding) so v1 fail-closed behaviour is preserved bit-for-bit. Activation lands in a follow-up PR once telemetry justifies it.
...
Until those preconditions are met, the v1 fail-closed behaviour stands. The scaffold seam is observable in CI / staging — `PdfOdtFallbackOrchestrator::isEnabled()` returns `false` in the default ship state.
