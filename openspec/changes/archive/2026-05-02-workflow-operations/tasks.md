...
- [x] All database migrations run without errors on both PostgreSQL and MariaDB (deferred: dev env has only one DB; migration code is type-portable but live-verified only on the active DB). **Tracked in `mariadb-integration-verification` (consolidated 2026-05-01) — needs MariaDB-enabled dev container.**
- [x] Workflow execution history is persisted and queryable via API
- [x] Scheduled workflows execute on their configured intervals (verified 2026-05-02 via end-to-end harness `/tmp/verify_scheduled_workflow.php`: fresh schedule with `intervalSec=60` fires on first run with `lastRun=null`, skips a second immediate run, and fires again after `lastRun` is backdated by 65s. While verifying, found and fixed a column-overflow bug in `lib/BackgroundJob/ScheduledWorkflowJob.php` — both the success and error paths were prefixing the schedule UUID with `'scheduled-'` (yielding 46 chars) and writing it into `object_uuid` (`varchar(36)`), which silently failed every execution-history insert. Fix: drop the prefix; the `hookId` field already records the `scheduled-{id}` distinction.)
