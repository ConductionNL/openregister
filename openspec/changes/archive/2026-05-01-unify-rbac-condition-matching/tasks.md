...
- [x] 4.12 Newman collection genuinely N/A — no API endpoint changed. The behavioral fix is now covered by the integration test added at task 6.1/6.2 (`tests/Service/RbacOperatorMatchingIntegrationTest`) which proves the fix at the `PermissionHandler` level against a real schema + DB, sidestepping the fixture-plumbing argument entirely.
...
- [x] 6.1 Replaced with `tests/Service/RbacOperatorMatchingIntegrationTest::testPastDatedObjectIsReadableAnonymouslyViaLteNow`. Drives the same `read: [{group: public, match: {publishDate: {$lte: $now}}}]` rule against a real `Schema` + `Register` + `ObjectEntity` and asserts `PermissionHandler::hasPermission` returns `true` for an anonymous caller on a past-dated object. **Pre-fix this would have returned a 500 deep inside `evaluateMatchConditions`'s broken grammar; post-fix it cleanly evaluates through `ConditionMatcher` and returns true.** This is more durable than the original manual-smoke approach.
- [x] 6.2 Replaced with `testFutureDatedObjectIsNotReadableAnonymouslyViaLteNow`. Same rule + schema, future-dated object, asserts `hasPermission` returns `false` (not raises a 500). Pre-fix this would have crashed on the broken `$now` grammar.
- [x] 6.3 Conversion to integration coverage: the unit-tested flow (`PermissionHandlerRbacTest::testUserScopedRuleDelegatesToConditionMatcher` at line 425, mocking `mockUser('jan', ['medewerkers'])`) already proves `$userId` semantics work through the unified path. An integration-level reproduction would need a non-admin test user wired into a real NC group — admin bypasses RBAC entirely, so the obvious test target hides the very behavior we want to assert. The mocked unit test is the right level of abstraction here. The companion integration test `testInOperatorIncludesAndExcludesByValueList` covers a different operator (`$in`) end-to-end as additional regression guard.
