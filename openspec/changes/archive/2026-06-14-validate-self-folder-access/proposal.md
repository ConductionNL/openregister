...
This change is one of two scaffolding changes that unblock the dossier-anonymisation pipeline. Anonymised dossiers are stored as objects in OpenRegister with files attached via `@self.folder`. Without this fix, an authenticated caller can bind an object — and its subsequent child file writes — to *any* user's folder by guessing or harvesting node IDs, producing a cross-tenant leak of pseudonymised records. The hardening lands in OpenRegister so every consumer of the save pipeline (DocuDesk's anonymisation flow first, others as they ship) inherits it. The sibling change `extend-schemas-in-register-service` fixes the schema-expansion contract that DocuDesk's anonymisation-routing UI depends on.

## Why

`@self.folder` is the only OpenRegister metadata field that lets a client hand us an arbitrary Nextcloud node ID and say "bind this object to that folder." Today the binding is unchecked: `FolderManagementHandler::createObjectFolderById()` calls `getExistingFolderFromProperty()` → `getNodeById()`, and `getNodeById()` deliberately falls back to `rootFolder->getById()` when the user-folder lookup misses. The root-folder fallback is intended to support anonymous/cross-user file reads, but when applied to folder *binding*, it means any authenticated caller can POST an object with `@self.folder: "<anyone-else's-folder-id>"` and silently attach their object — with all the files it subsequently creates — inside a folder they have no permission to read or write.
...
- **Audit-trail entry on rejection:** when a folder-access attempt is denied, emit an audit-trail entry with `action: "folder_access_denied"`, the attempted folder ID, and the actor. This change provides the forensic record only — rate-limiting, anomaly detection, and operator alerting on a probing burst are explicitly **out of scope** and tracked as a separate follow-up. The audit entry is the input to that downstream system; it is not itself the alarm.
...
- ADR-007 (Security and Auth): this change closes a multi-tenant escape hatch and aligns the `@self.folder` contract with the auth posture of the rest of the API — every write is governed by an explicit positive permission check on the acting user.
