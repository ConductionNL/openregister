...
## Purpose
Provide two reusable, additive OpenRegister foundation surfaces that leaf apps
(procest) consume: a public token-based "track your case" link to an object,
and a page-level pre-computed chart series render surface. Both are RBAC-scoped
and fail-closed.

**Standards**: ADR-005 (authorization / fail-closed), ADR-019 (pluggable
integration registry), ADR-022 (apps consume OR abstractions).
**Cross-references**: [pluggable-integration-registry](../../../changes/archive), `integration-shares`, `integration-analytics`.
