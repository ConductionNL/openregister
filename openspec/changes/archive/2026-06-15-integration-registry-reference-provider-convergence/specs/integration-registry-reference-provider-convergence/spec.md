...
status: draft
---
# Integration Registry ↔ IReferenceProvider Convergence Investigation

## Purpose

Operationalize ADR-041 Decision #3 by investigating whether and how the OpenRegister
integration registry (ADR-019 leaf system) should converge with Nextcloud's native
`OCP\Collaboration\Reference\IReferenceProvider` for the read/render half of its
responsibilities. This spec captures the **required outputs of the investigation** as
verifiable requirements so the spike's completion is checkable. It does NOT specify a registry
refactor — the deliverable is a decision record + recommendation.
