...
## Purpose
Make OpenRegister's app configuration import (`ImportHandler::importFromApp` → `importFromJson` → `importSeedData`) fault-tolerant at the **per-entity** granularity, and able to run **without a logged-in user** (the `occ`/installer/cron path). A single invalid entity (a name-missing register, a title-less seed object, a validation-failing mapping) MUST NOT abort the whole app's data-layer import: the app's registers and schemas MUST still be created, and the valid subset of objects MUST still be imported, with every skipped entity logged as a descriptive warning and counted for observability. Existing happy-path behaviour is preserved verbatim.
