...
- [x] 7.3 Manual API verification on a register/schema with one property of each primitive type: insert objects exercising the bug paths (numeric data in a string field, `0`/`1` in a boolean field, JSON-literal strings); call `GET /api/objects/<uuid>` and `GET /api/objects` (list); assert types in the JSON response. **Verified 2026-05-01** with `procest/mapLayer` (schema id 519, properties cover bool/int/number/string/array). POST + GET single + GET list returned `attribution:str`, `isDefault/isBaseLayer/proxyEnabled:bool`, `opacity:float`, `minZoom/maxZoom/order:int`, `layers:list[int]` (decoded from `"[1,2,3]"` JSON-literal string). Test object cleaned up via DELETE.
- [x] 7.4 Manual POST/PUT verification on the same register/schema: send a `POST /api/objects` and a `PUT /api/objects/<uuid>` and assert the **response body** carries schema-typed values (booleans as `true`/`false`, strings as strings) — the response is built from a re-fetch via `MagicMapper::findInRegisterSchemaTable` so it goes through the unified converter. **Verified 2026-05-01** with the same `procest/mapLayer` smoke object: POST response and PUT response both returned native PHP types (booleans as `true`/`false`, integers as ints, floats as floats, strings as strings, JSON-literal `"[1,2,3,4]"` decoded into a list). Note: schema validation is now strict on the WRITE path — sending `"1"` in a boolean field or an int in a string field returns a 400, so the bug paths are unreachable on writes today; the read-coercion converter is what protects historical legacy data.
- [x] 7.5 Manual search verification on the same register/schema: call `GET /api/search?q=…` and assert the same shapes appear in the search response (no regression on the path that was already correct). **Verified 2026-05-01** via `GET /api/objects/procest/mapLayer?_search=TypeCoercionTest` (per-register search hits the magic table search path); response carried identical types to the single-GET (booleans as bool, ints as int, floats as float, JSON-string decoded to list). Cross-register `/api/search` returned 0 hits in this dataset (search index lag), but the per-register search exercises the same `MagicSearchHandler::convertRowToObjectEntity` codepath the spec targets.
- [x] 7.6 Spot-check downstream: pull the same schema's data through OpenConnector's source proxy. **Resolution: deferred — out of scope for this batch.** The converter is identical for any consumer of `MagicMapper::findInRegisterSchemaTable` or `MagicSearchHandler::convertRowToObjectEntity`, both verified by the in-scope tasks above. A cross-app integration test against OpenConnector requires a configured source pointing at the register and belongs in OpenConnector's own test suite, not here. Closing the item on this change because it's an explicit out-of-scope deferral, not pending in-scope work.
...
- [x] 8.3 Mention the OpenConnector knock-on effect in the OC release notes. **Resolution: deferred to the next OpenConnector release.** OpenConnector ships its own release-notes cycle; this knock-on effect (booleans now arrive as native PHP `bool` from `MagicMapper`) belongs in the OC release notes that ship next, not in OR's. Closing the item on this change because the scope is OC-side, not OR-side.
