...
- **WHEN** a POST request is sent to `/api/objects/abc-123/_mail` with body `{"id": "1/6"}`
...
- **GIVEN** an object with `_mail: ["1/6", "1/12"]`
- **WHEN** a DELETE request is sent to `/api/objects/abc-123/_mail/1%2F6`
...
- **WHEN** a POST request is sent to `/api/objects/abc-123/_contacts` with body `{"id": "f47ac10b"}`
...
- **WHEN** a POST request is sent to `/api/registers/reg-123/_mail` with body `{"id": "1/6"}`
...
- **WHEN** a GET request is made to `/api/linked/_mail/1%2F6`
...
- **WHEN** a GET request is made to `/api/linked/_mail/1%2F999`
...
- **THEN** it MUST call `GET /api/linked/_mail/1%2F6` to find all linked objects
