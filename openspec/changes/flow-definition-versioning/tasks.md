...
- [x] 1.1 Migration: `version` (int, notnull, default 1) + `lifecycle_status`
...
- [x] 1.2 `lib/Db/FlowVersion.php` + `FlowVersionMapper` (find by flow+version,
...
- [x] 1.3 Repair step in the same migration: publish version 1 of every
...
- [x] 2.1 `FlowLifecycleGuard` — the preconditions, modelled on
...
- [x] 2.2 `FlowVersionService` — the transitions: publish (snapshot the head,
      ...
      or none.
- [x] 2.3 Trigger-set rebuild wired to publish/deprecate only:
...
- [x] 3.1 `FlowRunService::queue()` (`lib/Service/Flow/FlowRunService.php:321`)
...
- [x] 3.2 `refuseDeadEnd()` preflights the version being pinned, not
...
- [x] 3.3 ~~`FlowLocator::resolveFlow()` takes a version; memo key becomes
      flow + version.~~ **Solved differently, and better.** The memo caches the
      LIVE document, and the pinned graph is laid over it per run inside
      `FlowRunService::execute()` — so two runs on different versions in one
      worker batch each get their own graph without the resolver knowing about
      versions at all. `FlowVersionPinningTest::testTwoVersionsOfOneFlowAdvance
      InOneBatchWithoutCrossTalk` is the regression the task was written for.
...
- [x] 4.1 `SubFlowNode::execute()` (`lib/Service/Flow/Nodes/SubFlowNode.php:209`)
...
- [x] 5.1 `PUT /api/flows/{id}` (`appinfo/routes.php:539`) refuses a definition
      write against a published head with a 409 carrying a machine-readable
      reason; the stored graph is left untouched.
- [x] 5.2 New routes and controller methods: create-draft, publish, deprecate,
...
- [x] 5.3 Editor: `FlowDetailPage.vue` read-only for a published or deprecated
...
- [x] 6.2 Failure tests: a deleted pinned version fails the run with the
      version-specific message, never re-points it at a newer version, and
      never leaves it queued.
- [x] 6.3 Lifecycle, migration and regression: one published version per flow
