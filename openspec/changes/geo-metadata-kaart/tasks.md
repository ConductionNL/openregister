...
- [ ] **REQ-GEO-004 — Spatial queries in the API.** **HONEST STATUS (2026-05-02):** I previously claimed this was delivered via the existing `?_filter` mechanism. That claim was wrong — `?_filter` handles equality / range / in on scalar values, NOT GeoJSON bbox or within-polygon filters. No spatial filtering code exists in `lib/Service/` or `lib/Db/MagicMapper/` (verified by grep). Reverted. Real implementation needs a `GeoFilter` value object + a magic-mapper SQL clause builder that emits PostGIS `ST_Within` / `ST_Intersects` (or PHP-fallback bbox check on the JSON column).
- [ ] **REQ-GEO-011 — Geo-filtering in search and facets.** **HONEST STATUS (2026-05-02):** same fix as REQ-GEO-004 — this depends on the spatial-filter code path that doesn't exist. Reverted from the inappropriate closure.
