...
- [ ] **Request/Response Validation Against OAS Schema** — runtime validation of incoming request bodies against the generated OAS using `opis/json-schema`. NOT shipped. Needs a NC middleware that hooks into `before-controller` and validates the request body against the operation's schema. ~150 lines + tests.
- [ ] **Performance Impact of Validation / ETag caching** — the validation passes are trivially fast; the missing piece is ETag-based caching of OAS responses. NOT shipped. Needs a controller-level cache layer keyed on the generated-spec hash. ~50 lines + tests.
- [ ] **CI Redocly Lint Integration** — running `npx @redocly/cli lint` in CI. NOT shipped. Needs a `tests/integration/run-redocly-lint.sh` + a stable test-register fixture for reproducibility + a CI workflow step.
- [ ] **Schema Validation on Import** — pre-validating imported schemas in `ImportHandler` for OAS compatibility. NOT shipped. Needs `ImportHandler` to call the existing `OasService` validation pass on the imported schema and reject incompatible shapes early.
- [ ] **API-46 Problem Details (RFC 7807)** — enriching the `Error` schema with `type`, `title`, `status`, `detail`, `instance`. NOT shipped. Needs a structural change to `lib/Service/Resources/BaseOas.json` + a controller-side error response formatter that emits `application/problem+json` shape.
- [ ] **Strict-mode meta-schema validation** — running the generated OAS through the OpenAPI 3.1.0 JSON-Schema meta-schema. NOT shipped. Tracked in [issue #1378](https://github.com/ConductionNL/openregister/issues/1378) — blocker is vendoring the meta-schema document under `lib/Service/Resources/meta/` so the validator runs offline.
