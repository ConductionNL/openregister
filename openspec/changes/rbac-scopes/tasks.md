...
- [ ] **Custom Scope Definitions.** Not implemented — the scope vocabulary is fixed (action × entity-level). **Decision (2026-05-02):** ship option A — registers declare custom action verbs under `configuration.actions: [{name, description?}]`. `PermissionHandler` accepts them in rule lists, `OasService` emits them, `ScopesController` returns them. **Plus:** the validation pipeline MUST dispatch a `CustomScopeEvaluatingEvent` (and a paired `CustomScopeEvaluatedEvent` for telemetry) so consuming apps can hook in and contribute verdicts for their own action verbs. The event carries `(register, schema, action, user, object?)` and accepts a verdict from listeners; the first listener to return a non-null verdict wins, otherwise the static rule chain decides. The five canonical actions stay backward-compatible (no event dispatched on the hot path unless a custom action is being evaluated, to keep cost zero for existing rules). **Procest follow-up issue:** ZGW code in procest needs to be refactored from direct calls into event-based listeners now that custom scopes are evaluated through this dispatcher — tracked in [ConductionNL/procest#307](https://github.com/ConductionNL/procest/issues/307).

- [ ] **OAuth 2.0 Token Scopes Integration.** Not implemented — Nextcloud's OAuth2 layer is separate; OR doesn't translate RBAC rules into OAuth2 token scope requirements. **Decision (2026-05-02):** option B — owned by the `auth-system` spec, not by `rbac-scopes`. The OAuth2 token-scope translation is added as a requirement to the `auth-system` spec; this change closes off by documenting the boundary. The OAS already emits `security: [{ "oauth2": [groups] }, { "basicAuth": [] }]` per operation (Phase 3), so external proxies have machine-readable metadata to enforce against; the gap is internal binding from token-scope to RBAC verdict, which the auth-system spec now owns.
