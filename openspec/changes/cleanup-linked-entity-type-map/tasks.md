...
- [~] For each remaining reference, open a migration issue pointing to `IntegrationRegistry::listIds()` — handed off: the surviving references are documentation/ADR mentions describing the historical constant, not code that needs migration. No new issues opened.
...
- [~] PHPCS / PHPMD / PHPStan / Psalm strict pass — handed off to CI: local environment does not have PHPStan installed against PHP 8.3 (composer requires PHP >= 8.3 but only 8.2 is available locally). Touched files pass `php -l` syntax check; unit tests pass green (101 tests, 345 assertions including `SchemaLinkedTypesTest` regression).
...
- [~] No regression in integration discovery via `/api/integrations` or OCS capabilities — handed off to integration test run: discovery path is `IntegrationRegistry::list()` which this change did not touch.
