...
Import register schemas from external standards: Schema.org types
(properties, datatypes, descriptions, and a pre-filled JSON-LD
vocabulary mapping) and GGM (Gemeentelijk Gegevensmodel) objecttypes
(Dutch titles/definitions, attribute types, referentielijst enums) —
plus dialect detection on the existing schema upload path, import
provenance recorded on the schema, and a guarded update-from-source
flow. Backs the README "Schema Import — Import schemas from Schema.org,
OpenAPI, and GGM standards" promise; OpenAPI ingestion itself is
existing behaviour and is only labelled, not changed.
...
The schema ingestion path (`schemas#upload` / `schemas#uploadUpdate`)
