...
- [x] Migration: add `exif_metadata` JSON column to `openregister_file_links`
- [x] `PhotoService` — filter to images, lazy EXIF extraction
- [x] `PhotosController` — sub-resource endpoints (list, get with EXIF, link, unlink)
- [x] `PhotosProvider` — id='photos', label='Photos', icon='Image', group='docs', requiredApp='photos', storage='link-table'
- [x] Admin-setting: strip GPS from EXIF on link (default off)
