...
- [x] Confirm execution is reused, not reimplemented: reconciled `job` objects run via existing `openconnector/lib/Cron/JobTask.php` → `JobService::run()`; OR writes no execution/logging code (cron→`nextRun` computation is the only `nextRun` OR writes).
