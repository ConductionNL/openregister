...
| Reusable durability pattern | `lib/Db/WebhookLog(+Mapper)` append-only attempt rows + `lib/BackgroundJob/WebhookRetryJob.php` (300 s cadence, `next_retry_at`, retry-limit escalation, exponential backoff) + `lib/BackgroundJob/WebhookDeliveryJob.php` (QueuedJob carrying `attempt` in its argument). | The pattern OR-AD-2 reuses |
