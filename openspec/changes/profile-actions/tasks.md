...
- [x] Test: Integration test for password change flow — `tests/Service/UserProfileActionsIntegrationTest::testChangePasswordSucceedsAndOldPasswordStopsWorking` creates a real test user with a 15-char password (clears NC's 10-char policy), calls `UserService::changePassword`, then asserts the new password authenticates and the old one no longer does. Companion test `testChangePasswordRejectsWrongCurrentPassword` covers the wrong-current-password rejection path. Tests run end-to-end against a real `IUserManager` and clean up the user in tearDown.
- [x] Test: Integration test for data export — `testExportPersonalDataReturnsProfileShape` resets the rate-limit and calls `UserService::exportPersonalData`, asserting the response shape (`exportDate`, `profile.uid`). Companion test `testExportPersonalDataIsRateLimited` runs two consecutive exports and asserts the second hits the once-per-hour rate limit (RuntimeException with `retry_after` info).

  **Bonus production-bug fix uncovered by these tests:** `UserService::exportPersonalData` and `::getUserActivity` were both calling `AuditTrailMapper::findByActor()` — a method that did not exist on the mapper, so both endpoints would have crashed with `Call to undefined method` on production. Added `findByActor(string $userId, int $limit, int $offset, ?string $action, ?string $from, ?string $to): {results, total}` to `AuditTrailMapper`, wrapping `findEntities` with a `user = ?` filter and parallel COUNT(*) query for total. Verified by the integration tests passing without crashing on the audit-trail lookup.
