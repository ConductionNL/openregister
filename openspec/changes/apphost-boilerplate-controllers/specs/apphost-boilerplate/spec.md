...
# AppHost: Boilerplate Controllers and Bootstrap

## Purpose

Provide the standard Conduction app plumbing (dashboard/SPA serving, settings API, per-user preferences, install repair steps, admin settings panel, deep-link registration, route table) as parameterised classes in `OCA\OpenRegister\AppHost\`, registered into a leaf app with one `Bootstrap::register()` call, so leaf apps delete ~10 near-identical PHP classes each.

**Standards**: ADR-040 (AppHost), ADR-022 (apps consume OR abstractions), ADR-029 (route reachability)
**Cross-references**: `apphost-observability-engine` (sibling), per-app `adopt-apphost` changes
