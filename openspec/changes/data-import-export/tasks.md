...
- [ ] **Import MUST support progress tracking for large datasets.** Partial — the chunked-batch architecture documented in the `ImportService` class docblock provides progress hooks (the import is processed in chunks with summary aggregation), but a streaming `Server-Sent Events` progress endpoint surface is not exposed. The frontend currently polls the import job status. **Decision (2026-05-02):** chained on the `realtime-updates` spec — SSE plumbing belongs there, and import will subscribe as one more producer. Carve this requirement out of `data-import-export` and reintroduce it as a delta on top of `realtime-updates` once that capability ships. Polling stays as the documented behaviour until then.
