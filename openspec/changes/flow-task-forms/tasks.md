...
- [x] 1.1 `TransitionEngine::availableActions()` (`lib/Service/Lifecycle/TransitionEngine.php:457-484`)
      ...
      untouched.
- [x] 1.2 Make `resolveTransitionInputs()` (`:697-729`) reachable from a
      ...
      reading the code.
- [x] 1.3 Write the `inputs` contract into `openspec/specs/object-lifecycle/`
...
- [x] 2.1 The `openregister.user-task` node's `configForm()` gains the `form`
...
- [x] 2.2 `validateConfig()` refuses, naming schema + field + reason: a field
...
- [x] 2.3 `validateConfig()` refuses `kind: external` when the Forms app is
...
- [x] 3.1 `lib/Service/Task/TaskFormResolver.php` — a task with a `run_uuid`
      ...
      version.
- [x] 3.2 The resolver intersects the declaration with the LIVE subject
      ...
      stored).
- [x] 3.3 Expose the resolved form on the task read so the completion surface
...
- [x] 4.1 Completion with a form that names an action calls
...
- [x] 4.2 Ordering and failure: the object write commits BEFORE the task is
...
- [x] 4.3 Both authorizations apply and either may refuse: the task verb's
      (`flow-task-entity`) and the object write's. Being the assignee grants
      no write on the subject.
- [x] 4.4 Checklist completion stays on the task's own verbs and never enters
...
- [x] 6.1 Contract tests: an undeclared key, a missing required input, an
...
- [x] 6.2 Binding tests: a declaration whose `required` disagrees with
...
- [x] 6.3 Versioning and regression: an open task keeps its form across a
