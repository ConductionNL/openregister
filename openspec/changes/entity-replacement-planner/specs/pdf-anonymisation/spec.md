...
- **`complete` widens, `residual_count` does not.** Per `entity-replacement-planner`, a report carries two kinds of finding — `unmatched` (text may remain) and `partial` (split-matched, text gone) — and BOTH set `complete: false`. `residual_count` MUST continue to count only `unmatched` findings, so its meaning for existing consumers is unchanged. Consumers MUST NOT read `complete: false` as "PII remains in the output"; they MUST consult the finding kind.
