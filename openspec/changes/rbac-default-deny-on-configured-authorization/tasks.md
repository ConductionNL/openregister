...
- [ ] 4.4 Run `composer check:strict` and the affected PHPUnit/Newman suites in-container (`master-nextcloud-1`); regression-check against opencatalogi and softwarecatalog registers. NOTE: dev-deps are intentionally `--no-dev` in this checkout, so `check:strict`/PHPUnit cannot run locally — `php -l` passes on every changed file; full suite run is a container/CI step.
