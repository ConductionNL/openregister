...
- [ ] 7.6 Spot-check downstream: pull the same schema's data through OpenConnector's source proxy (one register-backed flow) — confirm booleans now arrive as `true`/`false` and strings stay strings. (Deferred: requires a configured OpenConnector source pointing at the register; cross-app integration test, out of scope for this batch — the converter is identical for any consumer of `MagicMapper::findInRegisterSchemaTable` or `MagicSearchHandler::convertRowToObjectEntity`, both verified above.)
