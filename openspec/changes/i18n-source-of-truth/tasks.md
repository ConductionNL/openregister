...
- [~] Add `sourceLanguage: string` to the JSON-Schema validator for — deferred to downstream cycle / fleet-wide adoption (handoff)
      property definitions in `lib/Db/Schema.php` (translatable
      properties may declare it; non-translatable MUST NOT).
- [~] Add migration `lib/Migration/Version1Date20260520120000.php` — — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Update `lib/Db/Translation.php` entity — add `sourceLanguage` — deferred to downstream cycle / fleet-wide adoption (handoff)
      column property, getter/setter via QBMapper.
- [~] Update `lib/Db/TranslationMapper.php` to include — deferred to downstream cycle / fleet-wide adoption (handoff)
      `source_language` in find/insert/update queries.
- [~] Add a back-fill `php occ openregister:translations:backfill-source-language` — deferred to downstream cycle / fleet-wide adoption (handoff)
      idempotent command (re-runs are no-ops once every row has a
      non-empty `source_language`).

## Phase 2 — Projection + status service

- [~] Update `lib/Service/TranslationProjectionService.php` to populate — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Add `lib/Service/TranslationStatusService::markDerivedTranslationsOutdated(string $objectUuid, string $property, string $sourceLanguage): int` — deferred to downstream cycle / fleet-wide adoption (handoff)
      that flips every non-source-language Translation row to status
      `outdated`. Returns count of rows flipped.
- [~] Wire the source-change trigger in — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Extend `lib/Controller/TranslationController::search` to honour — deferred to downstream cycle / fleet-wide adoption (handoff)
      `?sourceLanguage=<bcp47>`, `?isOutOfDate=true`,
      `?compareToSource=true` query filters.
- [~] Update `lib/Service/Object/RenderObject.php` to attach — deferred to downstream cycle / fleet-wide adoption (handoff)
      `_meta.languageMeta.<property> = { served, sourceLanguage,
      isSource, status }` when `?_translationMeta=true` is requested.
- [~] Set `X-Source-Language` response header in — deferred to downstream cycle / fleet-wide adoption (handoff)
      `lib/Middleware/LanguageMiddleware::afterController()` based on
      the resolved source language for the response.

## Phase 4 — Spec + tests

- [~] Write `specs/i18n-source-of-truth/spec.md` with one Requirement — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Add `tests/Unit/Service/TranslationProjectionServiceSourceLanguageTest.php` — deferred to downstream cycle / fleet-wide adoption (handoff)
      covering: object-level override, schema-default inheritance,
      register-default fallback.
- [~] Add `tests/Unit/Service/TranslationStatusServiceOutdatedTest.php` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Add `tests/Unit/Service/Object/RenderObjectLanguageMetaTest.php` — deferred to downstream cycle / fleet-wide adoption (handoff)
...
- [~] Wire the new Newman collection into — deferred to downstream cycle / fleet-wide adoption (handoff)
      `tests/newman/run-all.sh::DOMAIN_ORDER` after `relations`.

## Phase 5 — Documentation

- [~] Update `docs/i18n.md` (or create) describing the source-of-truth — deferred to downstream cycle / fleet-wide adoption (handoff)
