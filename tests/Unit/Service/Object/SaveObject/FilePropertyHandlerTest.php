...
class FilePropertyHandlerTest extends TestCase {
	...
	/**
	 * The SSRF guard runs BEFORE the request, not after it.
	 *
	 * testFetchFileFromUrlRefusesALoopbackAddress above already proves the
	 * address is rejected; it cannot prove nothing was fetched, because with no
	 * wrapper in place there is no way to observe a request that did happen.
	 * Here the fake wrapper is armed with content and then asserted never to
	 * have been opened — a guard that fetched first and validated afterwards
	 * would still be a read-SSRF, and would pass every other test in this file.
	 *
	 * @return void
	 */
	public function testProcessSingleFilePropertyRefusesTheUrlWithoutFetchingIt(): void {
