 ...
 */
function req(name, method, path, body, test, pre) {
	return {
		name,
		event: [
			...(pre
...
const setup = {
	...
	item: [
		...
		req(
			...
			{
				title: 'flow-engine-{{stamp}}',
				slug: 'flow-engine-{{stamp}}',
				description: 'flow engine coverage',
			},
			[
		...
		req(
			'create mapping',
			'POST',
			'/apps/openregister/api/mappings',
			{
		...
		req(
			...
			'GET',
			'/apps/openregister/api/mappings/{{mappingId}}',
			undefined,
			[
		...
		req(
			'create pager sub-flow',
			'POST',
			'/apps/openregister/api/flows',
			{
				name: 'flow-engine-pager-{{stamp}}',
				description:
					'Fetches one page and emits an item per record. Empty when the pages run out.',
				app: 'openregister',
				enabled: true,
				executionMode: 'async',
				nodes: [
					{ id: 'p1', type: 'openregister.trigger-manual', config: {} },
					{
						...
						config: {
							method: 'GET',
							source: '{{extSource}}',
							endpoint: '/posts?_limit=10&_page={{ iteration.index }}',
						},
					},
					{
						id: 'p3',
		...
		req(
			'create sync sub-flow',
			'POST',
			'/apps/openregister/api/flows',
			{
				name: 'flow-engine-sync-{{stamp}}',
				description:
					'Upserts one page of records. Called once per page by the paginated sync.',
				app: 'openregister',
				enabled: true,
				executionMode: 'async',
				nodes: [
		...
		req(
			'create summariser agent',
			'POST',
			'/apps/openregister/api/objects/hermiq/agent',
			{
	...
	return {
		...
		item: [
			...
			req(
				`${c.key} — run flow (sync)`,
				'POST',
				`/apps/openregister/api/flows/{{${v}_uuid}}/run`,
				{ sync: true },
				[
...
const coverage = {
	...
	item: [
		req(
			'report which cases ran',
			'GET',
			'/apps/openregister/api/flows?limit=1',
			undefined,
			[
...
const teardown = {
	...
	item: [
		req(
			'delete flows',
			'GET',
			'/apps/openregister/api/flows?limit=200',
			undefined,
			[
				'const body = pm.response.json()',
		...
		req(
			'delete one flow',
			'DELETE',
			'/apps/openregister/api/flows/{{deleteHead}}',
			undefined,
			[
		...
		req(
			'delete one object',
			'DELETE',
			'/apps/openregister/api/objects/{{register}}/{{schema}}/{{objectHead}}',
			undefined,
			[
		...
		req(
			'delete agent',
			'DELETE',
			'/apps/openregister/api/objects/hermiq/agent/{{agent}}',
			undefined,
			[
		...
		req(
			'no run history survives its flow',
			'GET',
			'/apps/openregister/api/flow-runs?limit=1',
			undefined,
			[
...
const collection = {
	info: {
		...
		description:
			'Executes eight real flows end to end and asserts what each one CHANGED, not merely that it reported success. Needs a FlowRunWorker alongside it: use tests/newman/run-flow-engine.sh.',
...
console.log(
	`wrote ${out} — ${CASES.length} cases, ${collection.item.length} folders`,
)
