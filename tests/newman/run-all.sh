...
declare -A DOMAIN_COLLECTIONS=(
    ...
    [i18n-source-of-truth]="$REPO_ROOT/tests/integration/openregister-i18n-source-of-truth.postman_collection.json"
    [i18n-api-language-negotiation]="$REPO_ROOT/tests/integration/openregister-i18n-api-language-negotiation.postman_collection.json"
    ...
    # A run's DECLARED subject set, and a task attached to one. Registered
    # here rather than left to a Playwright spec in tests/e2e/api-direct/,
    # for the same reason as `delegation` and `register-descriptors` below:
    # playwright.config.ts excludes `**/api-direct/**` from every project,
    # so such a spec runs only when a developer invokes an ad-hoc config by
    # hand and CI executes none of it. A green spec and zero CI coverage
    # look identical from the outside.
    #
    # Generated by tests/newman/build-flow-subjects-collection.mjs.
    [flow-subjects]="$REPO_ROOT/tests/newman/openregister-flow-subjects.postman_collection.json"
