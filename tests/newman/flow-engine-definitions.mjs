...
export const CASES = [
	{
		...
		description:
			'Fetch a collection from an external API and write each item as a local object.',
		nodes: [
	...
	},
	{
		...
		description:
			'Read a synced object, move it to reviewed, and raise a Nextcloud notification.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'r1',
			...
			},
			{
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'update',
					match: [{ property: 'sourceId', value: '{{ sourceId }}' }],
			...
			},
			{
				...
				config: {
					...
					body: {
						shortMessage: 'Flow engine test: an object was reviewed',
					},
				},
			},
			{ id: 'e1', type: 'openregister.end', config: {} },
	...
	},
	{
		...
		description:
			'Fetch messages, hand them to an agent for a one-line summary, store the result.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'call1',
			...
			},
			{
				...
				config: {
					agent: '{{agent}}',
					prompt: 'Summarise these messages in one short sentence.',
				},
			},
			{
				id: 'w1',
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'create',
					fields: { name: 'mailbox-summary', status: 'summarised' },
				},
			},
			{ id: 'e1', type: 'openregister.end', config: {} },
		],
	...
	},
	{
		...
		description:
			'Walk the register on a schedule and flag every record that fails a rule.',
		nodes: [
			{
				id: 't1',
			...
			// silently filters nothing is the quietest way to break a pipeline.
			{
				...
				config: { condition: { '==': [{ var: 'json.status' }, 'synced'] } },
			},
	...
	},
	{
		...
		description:
			'Look a record up in an external source and store the derived field.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'call1',
			...
			// 'No mapping matches "Array"'. Inline derivation is set-fields.
			{
				id: 'm1',
				type: 'openregister.set-fields',
				config: { set: { note: '{{ response.body.company.name }}' } },
			},
			{
				id: 'w1',
				type: 'openregister.object-write',
	...
	},
	{
		...
		description:
			'Branch on a record state and raise an approval task down the matching path.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'r1',
			...
			{ id: 'sw1', type: 'openregister.switch', config: {} },
			{
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'create',
					fields: { name: 'approval-task', status: 'awaiting-approval' },
				},
			},
			{ id: 'e1', type: 'openregister.end', config: {} },
		],
	...
	},
	{
		...
		description:
			'Loops pages of an external API and hands each page to a sync sub-flow, upserting so a re-run converges.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'pages1',
				...
				config: {
					...
					source: {
						...
						config: { flowId: '{{pagerFlow}}', wait: true },
					},
					body: [
						{
							type: 'openregister.sub-flow',
							config: { flowId: '{{syncFlow}}', wait: true },
						},
					],
					maxIterations: 15,
					onLimit: 'fail',
		...
		edges: [
			{ id: 'a', from: 't1', to: 'pages1' },
			{ id: 'b', from: 'pages1', to: 'e1' },
		],
	...
	},
	{
		...
		description:
			'Routes each record to a different branch by a rule, writes a different outcome on each, and merges the branches back into one list.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'r1',
			...
			// sends its items nowhere, silently.
			{
				...
				config: {
					rules: [
						{
							condition: { '>': [{ var: 'json.externalId' }, 50] },
							output: 'hi1',
						},
					],
					default: 'lo1',
			...
			},
			{
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'create',
					fields: { name: 'high', status: 'routed-high' },
				},
			},
			{
				id: 'lo1',
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'create',
					fields: { name: 'low', status: 'routed-low' },
				},
			},
		...
		expect: {
			status: 'routed-high',
			atLeast: 1,
			also: { status: 'routed-low', atLeast: 1 },
		},
	...
	},
	{
		...
		description:
			'Stores where the last sync got to, reads it back, and records it — the state a real incremental sync keeps.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'put1',
				type: 'openregister.flow-state',
				config: { operation: 'set', key: 'lastSyncedId', value: 100 },
			},
			{
				id: 'get1',
				type: 'openregister.flow-state',
		...
		edges: [
			{ id: 'a', from: 't1', to: 'put1' },
			{ id: 'b', from: 'put1', to: 'get1' },
			{ id: 'c', from: 'get1', to: 'w1' },
			{ id: 'd', from: 'w1', to: 'e1' },
		],
		...
		expect: {
			status: 'cursor-stored',
			atLeast: 1,
			field: { name: 'externalId', equals: 100 },
		},
	},
	{
		key: 'batch-export',
		...
		description:
			'Read local objects and POST them outward in fixed-size batches.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'r1',
		...
		edges: [
			{ id: 'a', from: 't1', to: 'r1' },
			{ id: 'b', from: 'r1', to: 'b1' },
			{ id: 'c', from: 'b1', to: 'call1' },
			{ id: 'd', from: 'call1', to: 'e1' },
		],
	...
	},
	{
		...
		description:
			'Feed fields into a stored mapping and write the RESHAPED result, proving the map node transformed rather than passed through.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 's1',
				type: 'openregister.set-fields',
				config: { set: { first: 'Ada', last: 'Lovelace', n: '7' } },
			},
			...
			{
				id: 'm1',
				type: 'openregister.map',
				config: { mapping: '{{mappingUuid}}' },
			},
			{
				id: 'w1',
				type: 'openregister.object-write',
		...
		expect: {
			status: 'mapped',
			atLeast: 1,
			text: { name: 'name', equals: 'Ada Lovelace' },
		},
	},
	{
		key: 'trigger-object',
		...
		description:
			'A flow whose entry point is an object event still runs when triggered manually.',
		nodes: [
			{
				id: 'to1',
			...
			},
			{
				...
				config: {
					...
					schema: '{{schema}}',
					operation: 'create',
					fields: { name: 'event-triggered', status: 'evented' },
				},
			},
			{ id: 'e1', type: 'openregister.end', config: {} },
		],
	...
	},
	{
		...
		description:
			'A wait step parks the token instead of finishing, and says so.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
		...
		expect: {
			terminal: ['suspended'],
			step: { node: 'wa1', status: 'suspended' },
		},
	},
	{
		...
		description:
			'Hand a whole configured synchronization to one node and let it import the collection.',
		nodes: [
			{ id: 't1', type: 'openregister.trigger-manual', config: {} },
			{
				id: 'sr1',
				type: 'openconnector.synchronization-run',
				config: { synchronization: '{{synchronization}}' },
			},
			{ id: 'e1', type: 'openregister.end', config: {} },
		],
		edges: [
