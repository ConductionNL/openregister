 ...
 * WHAT THIS COVERS
 * ----------------
 * A run declares what it is working on, and a task hangs on it. The scene, over
 * the live HTTP API: a case is created and recorded under the role `case`, the
...
const scene = {
	...
	item: [
		...
		runFlow('the whole scene', 'sceneFlow', 'sceneRun', [
			'// A cascade guard AND a diagnostic: every later assertion in this',
			'// folder reads something this run was supposed to produce, so when the',
			'// run fails they all fail for a reason none of them names. Putting the',
			'// run body in THIS message is what makes the next red run readable.',
			"pm.test('the step suspended waiting for its answer', () => {",
			"    const why = JSON.stringify(run).slice(0, 900)",
			"    pm.expect(String(run.status), 'a step that raised a task must suspend; the run said: ' + why).to.eql('suspended')",
			'})',
		]),
		req(
	...
	item: [
		...
		req(
			...
			[
				...
				"pm.test('an empty set, never a missing key and never an array', () => {",
				'    // `json_encode` turns an empty PHP array into `[]`, so a run that',
				'    // declared nothing would serve a JSON ARRAY where a populated one',
				'    // serves a MAP. A typed client cannot read both.',
				"    pm.expect(body.subjects, 'subjects must be present').to.be.an('object')",
				"    pm.expect(Array.isArray(body.subjects), 'subjects must not be an array').to.eql(false)",
...
const teardown = {
	...
	item: [
		...
		req(
			...
			undefined,
			[
				'// 409 IS A CORRECT REFUSAL, NOT A FAILED TEARDOWN. The runs created',
				'// case objects in this register, and a register holding objects is',
				'// not deletable — which is the behaviour we want everywhere else.',
				'// Accepting it here is safe because every fixture is namespaced with',
				'// {{stamp}}, so nothing a later run does can collide with what is',
				"// left, and CI destroys the whole container when the job ends. A 5xx",
				'// still fails: that would be the register read breaking, not a',
				'// register declining to disappear under its own contents.',
				"pm.test('the register is gone, or declined because it still holds objects', () => {",
		...
		req(
			...
			undefined,
			[
				'// Same reasoning as the register above.',
				"pm.test('the schema is gone, or declined because objects still use it', () => {",
				'    pm.expect([200, 204, 404, 409]).to.include(pm.response.code)',
				'})',
			],
		),
...
const collection = {
	info: {
		...
		description:
			...
			+ 'Asserts the recording, the attachment and the refusal; the branch the '
			+ 'run takes after the answer is the flow-engine collection\'s subject. '
