...
test.describe('schema-crud — create→read→update→delete with property persistence', () => {
	...
	test('READ — schema lists as a real row and exposes both properties', async ({ page, request }) => {
		test.skip(schemaId === null, 'no schema created')

		// API persistence: GET returns the two properties.
		const get = await request.get(`${API}/schemas/${schemaId}`, { headers: { Accept: 'application/json' } })
		expect(get.status()).toBe(200)
		const body = await get.json()
		expect(Object.keys(body.properties ?? {})).toEqual(expect.arrayContaining(['title', 'count']))
		...
		await expect(page.getByText(SCHEMA_TITLE, { exact: false }).first())
			.toBeVisible({ timeout: 20_000 })
	})

	test('UPDATE — add a third property and assert it persisted + re-renders', async ({ page, request }) => {
		test.skip(schemaId === null, 'no schema to update')

		const threeProps = {
			...twoPropertySchema(),
			tag: { type: 'string', title: 'Tag', description: 'Added by update step' },
		}
		...
		const reget = await request.get(`${API}/schemas/${schemaId}`, { headers: { Accept: 'application/json' } })
	...
	test('DELETE — remove the schema and assert it is gone', async ({ page, request }) => {
		test.skip(schemaId === null, 'no schema to delete')

		const del = await request.delete(`${API}/schemas/${schemaId}`, { headers: { Accept: 'application/json' } })
		expect(del.status(), 'DELETE /api/schemas/{id}').toBe(200)

		const gone = await request.get(`${API}/schemas/${schemaId}`, { headers: { Accept: 'application/json' } })
		expect(gone.status(), 'deleted schema should not return 200').toBeGreaterThanOrEqual(400)

		await page.goto(SCHEMAS_ROUTE, { waitUntil: 'domcontentloaded' })
		await expect(page.locator('[data-testid="cn-index-page"]')).toBeVisible({ timeout: 30_000 })
		await expect(page.getByText(SCHEMA_TITLE, { exact: false }))
	...
	test.afterAll(async ({ request }) => {
		if (schemaId === null) {
			const resp = await request.get(`${API}/schemas?_limit=200`, { headers: { Accept: 'application/json' } }).catch(() => null)
			if (resp && resp.ok()) {
				const s = (await resp.json()).results?.find((x: any) => x.slug === SCHEMA_SLUG)
				schemaId = s?.id ?? null
			}
		}
		if (schemaId !== null) {
