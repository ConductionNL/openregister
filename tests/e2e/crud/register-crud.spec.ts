...
test.describe('register-crud — full create→read→update→delete with persistence', () => {
	...
	async function findOurRegister(request: APIRequestContext): Promise<Record<string, any> | null> {
		const resp = await request.get(`${API}/registers?_limit=200`, { headers: { Accept: 'application/json' } })
		if (!resp.ok()) return null
		const body = await resp.json()
		return (body.results ?? []).find((r: any) => r.slug === REG_SLUG) ?? null
	}

	test('CREATE a register through the UI and assert it persisted', async ({ page, request }) => {
		...
		const submit = page.locator('.modal-container').getByRole('button', { name: /^(Create|Save)$/ }).last()
		await expect(submit).toBeVisible({ timeout: 10_000 })
		await submit.click()

		// Dialog closes on success (the form-dialog content unmounts).
		await expect(dialog).toBeHidden({ timeout: 15_000 })
		await page.locator('.modal-container').getByRole('button', { name: /^(Create|Save)$/ })
		...
		}, { timeout: 15_000, message: 'created register should be persisted with the submitted title' })
			.toBe(REG_TITLE)

		expect(registerId, 'register id resolved from API').toBeTruthy()
	})

	test('READ — the new register renders as a real row in the registers table', async ({ page }) => {
		test.skip(registerId === null, 'create step did not persist a register')
		await page.goto(REGISTERS_ROUTE, { waitUntil: 'domcontentloaded' })
		await expect(page.locator('[data-testid="cn-index-page"]')).toBeVisible({ timeout: 30_000 })

		// The register title must be rendered somewhere in the list (table row
		// or card), NOT just an empty state.
		await expect(page.getByText(REG_TITLE, { exact: false }).first())
			.toBeVisible({ timeout: 20_000 })
	})

	test('UPDATE — edit the title and assert persistence + re-render', async ({ page, request }) => {
		...
		const put = await request.put(`${API}/registers/${registerId}`, {
			...
			data: { slug: REG_SLUG, title: REG_TITLE_UPDATED, description: 'updated by e2e' },
		})
		...
		await page.goto(REGISTERS_ROUTE, { waitUntil: 'domcontentloaded' })
		await expect(page.locator('[data-testid="cn-index-page"]')).toBeVisible({ timeout: 30_000 })
		await expect(page.getByText(REG_TITLE_UPDATED, { exact: false }).first())
			.toBeVisible({ timeout: 20_000 })
	})

	test('DELETE — remove the register and assert it is gone from API and list', async ({ page, request }) => {
		test.skip(registerId === null, 'no register to delete')

		const del = await request.delete(`${API}/registers/${registerId}`, { headers: { Accept: 'application/json' } })
		expect(del.status(), 'DELETE /api/registers/{id}').toBe(200)

		// API: the register must no longer be retrievable.
		const gone = await request.get(`${API}/registers/${registerId}`, { headers: { Accept: 'application/json' } })
		...
		await page.goto(REGISTERS_ROUTE, { waitUntil: 'domcontentloaded' })
		await expect(page.locator('[data-testid="cn-index-page"]')).toBeVisible({ timeout: 30_000 })
		await expect(page.getByText(REG_TITLE_UPDATED, { exact: false }))
	...
	test.afterAll(async ({ request }) => {
		if (registerId === null) {
			...
			const resp = await request.get(`${API}/registers?_limit=200`, { headers: { Accept: 'application/json' } }).catch(() => null)
			if (resp && resp.ok()) {
				const reg = (await resp.json()).results?.find((r: any) => r.slug === REG_SLUG)
				registerId = reg?.id ?? null
			}
		}
		if (registerId !== null) {
