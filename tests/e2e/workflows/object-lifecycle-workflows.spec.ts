...
test.describe('workflow: audit-trail records create and update', () => {
	...
	test.beforeAll(async ({ request }) => {
		...
		schema = await createSchema(request, `${RUN_ID}-audit`, 'sch', twoPropertySchema())
		await linkSchemaToRegister(request, register, [schema.id])
		const obj = await createObject(request, register.id, schema.id, { title: `Audit ${RUN_ID}`, count: 1 })
		objectId = obj.id
	})
	...
	async function getTrails(request: APIRequestContext): Promise<Array<Record<string, any>>> {
	...
	test('a "create" audit-trail entry exists after object creation', async ({ request }) => {
		const trails = await getTrails(request)
		expect(trails.length, 'at least one audit entry after create').toBeGreaterThanOrEqual(1)
		const actions = trails.map(t => t.action)
		expect(actions, 'a create action must be recorded').toContain('create')
	})

	test('updating the object records a new "update" audit-trail entry', async ({ request }) => {
		...
		await expect.poll(async () => {
			...
			return after.some(t => t.action === 'update') && after.length > before.length
		}, { timeout: 15_000, message: 'an update audit-trail entry should appear after editing' }).toBe(true)
	})

	test('UI: the audit-trails view renders entries (data-dependent)', async ({ page, request }) => {
		...
		const actionCell = page.locator('table tbody tr td', { hasText: /CREATE|UPDATE/ })
...
test.describe('workflow: soft-delete then restore', () => {
	...
	test.beforeAll(async ({ request }) => {
		...
		schema = await createSchema(request, `${RUN_ID}-trash`, 'sch', twoPropertySchema())
		await linkSchemaToRegister(request, register, [schema.id])
		const obj = await createObject(request, register.id, schema.id, { title: `Trash ${RUN_ID}`, count: 5 })
		objectId = obj.id
	})
	...
	test('deleting an object makes its direct GET return 404 (soft-delete applied)', async ({ request }) => {
		...
		const gone = await request.get(`${API}/objects/${register.id}/${schema.id}/${objectId}`, {
			headers: { Accept: 'application/json' },
		})
	...
	test('BUG-1: the soft-deleted object appears in GET /api/deleted', async ({ request }) => {
		...
		const body = await resp.json()
		const found = (body.results ?? []).some((r: any) => JSON.stringify(r).includes(objectId))
	...
	test('BUG-2: restoring brings the object back into its register/schema', async ({ request }) => {
		...
		const back = await request.get(`${API}/objects/${register.id}/${schema.id}/${objectId}`, {
			headers: { Accept: 'application/json' },
		})
