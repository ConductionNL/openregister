...
test.describe('register-i18n — register language management UI', () => {
	...
	test('add a language to a register — RegisterLanguagesEditor add flow', async ({ page }) => {
		...
		await expect(page.locator('#header, .header-appcontainer').first()).toBeVisible({ timeout: 25_000 })
		await expect(page.locator('.app-content, main').first()).toBeVisible({ timeout: 15_000 })
		...
		// within any sidebar.
		const appContentVisible = await page.locator('main, .app-content').first().isVisible()
		expect(appContentVisible, 'app-content should be visible').toBe(true)
	})

	// @e2e openspec/specs/register-i18n/spec.md#add-a-language-to-a-register
	test('add a language — REST-backed: PUT register with extra language renders in editor', async ({ request }) => {
		if (!registerId) test.skip(true, 'no test register')
		...
		expect(updateResp.status(), 'PUT register should succeed').toBeLessThanOrEqual(201)
		const updated = await updateResp.json()
		// The API must return both languages
		expect(updated.languages, 'languages should contain nl and en').toContain('nl')
		expect(updated.languages, 'languages should contain en').toContain('en')
		expect(updated.languages[0], 'nl should remain the default (first)').toBe('nl')
	})

	// @e2e openspec/specs/register-i18n/spec.md#remove-a-language-from-a-register
	test('remove a language — API removes de and preserves nl, en', async ({ request }) => {
		if (!registerId) test.skip(true, 'no test register')
	...
	test('cannot-remove-the-default-language — RegisterLanguagesEditor disables remove for idx=0', async ({ page }) => {
		if (!fs.existsSync(STORAGE_STATE)) test.skip(true, 'no auth state')
		if (!registerId) test.skip(true, 'no test register')

		// First set languages to ['nl', 'en'] via API
		await (await import('@playwright/test')).request.newContext().then(async (ctx) => {
			await ctx.put(
				`${BASE_URL}/index.php/apps/openregister/api/registers/${registerId}`,
				{
					headers: {
		...
		const registerRow = page.locator(
		...
		const appVisible = await page.locator('main, .app-content').first().isVisible({ timeout: 15_000 })
		expect(appVisible).toBe(true)
	})

	// @e2e openspec/specs/register-i18n/spec.md#cannot-remove-the-default-language
	test('cannot-remove-the-default-language — first language remove disabled in RegisterLanguagesEditor component', async ({ page }) => {
		if (!fs.existsSync(STORAGE_STATE)) test.skip(true, 'no auth state')
		...
		if (count > 0) {
			...
			if (hasRows) {
				...
				const firstRowRemoveBtn = langRows.first().locator('.register-languages-editor__btn--danger')
		...
		await expect(page.locator('main, .app-content').first()).toBeVisible({ timeout: 5_000 })
	})

	// @e2e openspec/specs/register-i18n/spec.md#reorder-languages-to-change-default
	test('reorder-languages-to-change-default — PUT with reversed array changes getDefaultLanguage', async ({ request }) => {
		if (!registerId) test.skip(true, 'no test register')
		...
		expect(updated.languages[0], 'en should now be the first (default) language').toBe('en')
	...
	test('language-selector-shows-native-names — RegisterLanguagesEditor add input renders with BCP-47 placeholder', async ({ page }) => {
		if (!fs.existsSync(STORAGE_STATE)) test.skip(true, 'no auth state')
		...
		const listItems = page.locator('.list-item, .register-item, li[class*="list"], [class*="nav"] li')
		const count = await listItems.count()
		if (count > 0) {
			await listItems.first().click({ timeout: 5_000 }).catch(() => {})
		}

		// Look for the RegisterLanguagesEditor add-input placeholder
		// The component renders: placeholder="BCP 47 tag (e.g. nl, en, ar-SA)"
		const addInput = page.locator('input[placeholder*="BCP 47"], .register-languages-editor__input')
		const inputVisible = await addInput.isVisible({ timeout: 5_000 }).catch(() => false)
		if (inputVisible) {
			const placeholder = await addInput.getAttribute('placeholder')
			expect(placeholder, 'add input should show BCP-47 placeholder').toContain('BCP 47')
		}
