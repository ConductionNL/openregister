...
test.describe('saved-search-views — kanban + calendar presentation (REST)', () => {
	...
	// ─────────────────────────────────────────────────────────────────────
	test('POST /api/views persists a kanban presentation and GET returns it (REQ-VIEW-PRES-01)', async ({
		request,
	}) => {
		...
		expect(createResp.status(), 'POST /api/views (kanban)').toBeLessThanOrEqual(
			201,
		)
		...
		expect(
			fetchedView.presentation?.viewType,
			'presentation.viewType survives the round-trip',
		).toBe('kanban')
		expect(fetchedView.presentation?.kanban?.groupByField).toBe('status')
		expect(fetchedView.presentation?.kanban?.columnOrder).toEqual([
			'todo',
			'doing',
			'done',
		])
	...
	// ─────────────────────────────────────────────────────────────────────
	test('GET /api/views/:id/kanban returns columns in columnOrder with cards (REQ-VIEW-KANBAN-02)', async ({
		request,
	}) => {
		...
		expect(body.columns.map((c: { value: string }) => c.value)).toEqual([
			'todo',
			'doing',
			'done',
		])

		const todoColumn = body.columns.find(
			(c: { value: string }) => c.value === 'todo',
		)
		const doingColumn = body.columns.find(
			(c: { value: string }) => c.value === 'doing',
		)
		const doneColumn = body.columns.find(
			(c: { value: string }) => c.value === 'done',
		)
	...
	// ─────────────────────────────────────────────────────────────────────
	test('Guarded write moves a card between columns and it persists (REQ-VIEW-KANBAN-03)', async ({
		request,
	}) => {
		...
		const patchResp = await request.patch(
			`${API}/objects/${register.id}/${schema.id}/${movedId}`,
			{
				headers: JSON_HEADERS,
				data: { status: 'doing' },
			},
	...
	// ─────────────────────────────────────────────────────────────────────
	test('POST /api/views persists a calendar presentation (REQ-VIEW-PRES-01 / REQ-VIEW-CAL-04)', async ({
		request,
	}) => {
	...
	test('GET /api/views/:id/calendar returns objects in range carrying dueDate (REQ-VIEW-CAL-04)', async ({
		request,
	}) => {
		...
		for (const obj of body.objects as Array<{ dueDate?: string }>) {
			if (obj.dueDate) {
				expect(
					obj.dueDate >= '2026-08-01' && obj.dueDate <= '2026-08-31',
				).toBe(true)
			}
		}
	...
	// ─────────────────────────────────────────────────────────────────────
	test('POST /api/views rejects a kanban groupByField that is not a schema property (REQ-VIEW-PRES-01)', async ({
		request,
	}) => {
		const resp = await request.post(`${API}/views`, {
			headers: JSON_HEADERS,
		...
		expect(
			resp.status(),
			'invalid groupByField must be rejected',
