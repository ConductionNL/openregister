...
test.describe('OpenConnector chaining expressed as a flow', () => {
	...
	test('a multi-step chain runs in the declared order and is inspectable', async ({
		request,
	}) => {
		...
		expect(
			result.items?.[0]?.json?.step,
			'the second step ran after the first',
		).toBe('two')
		// Every hop is recorded, which recursive followUps never gave you.
		expect(
			Array.isArray(result.log),
			'the run carries an inspectable trace',
		).toBeTruthy()
		expect(result.log.length, 'the trace is not empty').toBeGreaterThan(0)
	})

	test('a self-referencing flow is bounded instead of recursing forever', async ({
		request,
	}) => {
		...
		const flow = await createFlow(request, 'Self cycle', {
			nodes: [{ id: 'a' }, { id: 'b' }],
			edges: [],
		})

		const selfEdge = await request.put(
			`${API}/objects/${flowReg}/${flowSch}/${flow}`,
			{
				headers: JSON_HEADERS,
				data: {
					...
					edges: [
						{
							...
							config: { flow, wait: true },
						},
					],
				},
			},
		)
		expect(selfEdge.status(), 'store the self-referencing edge').toBeLessThan(
			400,
		)
		...
		expect(
			elapsed,
			'a self-referencing chain terminates rather than hanging',
		).toBeLessThan(90_000)
		...
		if (run.ok()) {
			...
			expect(
				result.status,
				'a cycle must not be reported as a clean success',
			).not.toBe('completed')
		} else {
			expect(
				run.status(),
				'refused outright is equally correct',
