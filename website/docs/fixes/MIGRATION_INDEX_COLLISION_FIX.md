...
### Error 3: Foreign Key Type Mismatch
```
...
The third issue occurred in migration `Version1Date20251116000000` when creating the `openregister_entity_relations` table. The foreign key columns `entity_id` and `chunk_id` were defined as `BIGINT` without the `unsigned` attribute, but they referenced primary keys in `openregister_entities` and `openregister_chunks` tables that ARE defined as `BIGINT unsigned`. MySQL/MariaDB requires foreign key columns to exactly match the type and attributes of the referenced columns.
...
### Fix for Error 3: Foreign Key Type Matching

Updated the `openregister_entity_relations` table creation in migration `Version1Date20251116000000` to ensure foreign key columns match the referenced columns:

```php
$table->addColumn('entity_id', Types::BIGINT, [
    'notnull' => true,
    'unsigned' => true,  // Added to match openregister_entities.id
]);
$table->addColumn('chunk_id', Types::BIGINT, [
    'notnull' => true,
    'unsigned' => true,  // Added to match openregister_chunks.id
]);
```
...
- ✅ Ensures foreign key constraints are properly formed with matching column types
...
10. **Test foreign key constraints** separately before deploying migrations
