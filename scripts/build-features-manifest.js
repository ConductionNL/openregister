...
function build(cwd) {
	...
	for (const entry of entries) {
		...
		if (!meta || !meta.status) {
			console.warn(`[build-features-manifest] spec ${slug}: missing frontmatter or status — skipping`);
			continue;
		}
		...
		if (!title || !summary) {
			console.warn(`[build-features-manifest] spec ${slug}: missing title or purpose — skipping`);
			continue;
		}
		const docsUrl = buildDocsUrl({
			frontmatterUrl: meta.docsUrl, slug,
			host: remote?.host, owner: remote?.owner, repo: remote?.repo, branch,
		});
		features.push({ slug, title, summary, ...(docsUrl ? { docsUrl } : {}) });
	}
	features.sort((a, b) => a.title.localeCompare(b.title, undefined, { sensitivity: 'base' }));
	return { schemaVersion: 1, generatedAt: new Date().toISOString(), features };
}
...
function main() {
	...
	if (args.check) {
		...
		const existingNoTimestamp = existing.replace(/"generatedAt"\s*:\s*"[^"]+"/, '"generatedAt":"<ignored>"');
		const newNoTimestamp = serialized.replace(/"generatedAt"\s*:\s*"[^"]+"/, '"generatedAt":"<ignored>"');
		if (existingNoTimestamp === newNoTimestamp) {
			console.log('[build-features-manifest] docs/features.json is up-to-date.');
			process.exit(0);
		}
		console.error('[build-features-manifest] DRIFT detected — run `node scripts/build-features-manifest.js` and commit the result.');
		process.exit(1);
	}
	fs.mkdirSync(path.dirname(outPath), { recursive: true });
	fs.writeFileSync(outPath, serialized);
	console.log(`[build-features-manifest] wrote ${manifest.features.length} features to ${path.relative(args.cwd, outPath)}`);
}
...
module.exports = { build, parseFrontmatter, extractTitle, extractSummary, buildDocsUrl };
