...
module.exports = defineConfig([
	...
	// and PHPCS.
	//
	// It disables no CORRECTNESS rule: the `vue/no-deprecated-*` family from
	// `conductionVue3Fixes`, plus this app's own `vue/no-multiple-template-root`
	// and `vue/v-on-event-hyphenation` overrides above, are all still present and
	// still ON, because prettier has no opinion about them. `indent` is now off
	// HERE and enforced by prettier's `useTabs: true` instead — the same tab, from
	// the tool that also covers CSS and SCSS, which @nextcloud/stylelint-config no
	// longer does.
